...
def preprocess_for_residual_analysis(
    ...
    if ref_wl_col is None:
        wl_cols = [
            col for col in usgs_df.columns if col.endswith("_wl") and not col.startswith("gnss")
        ]
        if wl_cols:
    ...
    if "rh_std_m" in gnss_df.columns:
        ...
        if window >= 3:
            aligned_data["uncertainty"] = (
                aligned_data["difference"].rolling(window=window, center=True).std()
            )
        else:
            aligned_data["uncertainty"] = np.nan
...
def create_residual_analysis_plot(
    ...
    if plot_mode == "dual_axis":
        ...
        ax1.set_ylabel(
            "GNSS-IR Water Surface Elevation (m)",
            fontsize=12,
            fontweight="bold",
            color=colors.get("gnss", "#2E86AB"),
        )
        ...
        ax1_twin.plot(
            aligned_data["timestamp"],
            aligned_data["ref_water_level"],
            color=colors.get("usgs", "#A23B72"),
            linewidth=1.5,
            label=f"{reference_source} Water Level",
            alpha=0.8,
        )
        ax1_twin.set_ylabel(
            f"{reference_source} Water Level (m)",
            fontsize=12,
            fontweight="bold",
            color=colors.get("usgs", "#A23B72"),
        )
        ...
        ax1.plot(
            aligned_data["timestamp"],
            aligned_data["gnss_water_level"] - gnss_mean,
            color=colors.get("gnss", "#2E86AB"),
            linewidth=1.5,
            label=f"GNSS-IR WSE (mean removed: {gnss_mean:.2f} m)",
            alpha=0.8,
        )
        ax1.plot(
            aligned_data["timestamp"],
            aligned_data["ref_water_level"] - usgs_mean,
            color=colors.get("usgs", "#A23B72"),
            linewidth=1.5,
            label=f"{reference_source} Water Level (mean removed: {usgs_mean:.2f} m)",
            alpha=0.8,
        )
        ...
        ax1.plot(
            ...
            color=colors.get("usgs", "#A23B72"),
            linewidth=1.5,
            label=f"{reference_source} Water Level (normalized)",
            alpha=0.8,
        )
    ...
    ax1.set_title(
        f"{station_name} - Water Level Comparison{title_suffix}\n"
    ...
    ax2.plot(
        aligned_data["timestamp"],
        aligned_data["difference"],
        color=colors.get("difference", "#F18F01"),
        linewidth=1,
        label=f"GNSS - {reference_source} Difference",
        alpha=0.8,
    )
    ...
    if len(valid_data) > 0:
        ...
        seasonal_stats = valid_data_copy.groupby("season")["difference"].agg(
            ["mean", "std", "count"]
        )
        seasons = ["Winter", "Spring", "Summer", "Fall"]
        season_colors = [
            colors.get("gnss", "#2E86AB"),
            colors.get("usgs", "#A23B72"),
            colors.get("difference", "#F18F01"),
            "#8B4513",
        ]
        ...
        ax4.text(0.5, 0.5, "No seasonal data", ha="center", va="center", transform=ax4.transAxes)
    ...
    if len(valid_data) > 30:  # Need sufficient data for rolling stats
        ...
        rolling_rmse = np.sqrt(
            (valid_data["difference"] ** 2).rolling(window=window, center=True).mean()
        )
        ...
        ax5.text(
            0.5,
            0.5,
            "Insufficient data for rolling statistics",
            ha="center",
            va="center",
    ...
    fig.text(
        0.02,
        0.02,
        f'Processed: {metadata.get("processing_date", "Unknown")} | '
...
def render_yearly_residual_tab(
    rh_data, usgs_data, coops_data, selected_station, selected_year, erddap_data=None
):
    """
    ...
    **Comprehensive yearly view showing the complete time series comparison between GNSS-IR
    ...
    with col1:
        resolution = st.selectbox(
            "Temporal Resolution:",
            ["D", "6H", "3H", "H"],
            format_func=lambda x: {"D": "Daily", "6H": "6-Hourly", "3H": "3-Hourly", "H": "Hourly"}[
                x
            ],
            index=0,
    ...
    with col3:
        outlier_filter = st.checkbox(
            "Apply Outlier Filter", value=True, help="Remove statistical outliers before analysis"
        )
    ...
    if visualization_type == "Residual Analysis":
        ...
        rolling_window = st.slider(
            "Rolling window (days):", 7, 60, 30, key="yearly_timeline_window"
        )
    ...
    button_label = (
        ...
        else "🔄 Process Residual Analysis"
    )
    if st.button(button_label, type="primary"):
        if visualization_type == "Multi-Parameter Timeline":
            ...
            with st.spinner("Creating multi-parameter timeline..."):
                ...
                if (
                    (usgs_for_plot is None or usgs_for_plot.empty)
                    and coops_data is not None
                    and not coops_data.empty
                ):
                    ...
                    use_coops = st.checkbox(
                        "Use CO-OPS data instead of USGS", value=False, key="yearly_timeline_coops"
                    )
                    if use_coops:
                        water_level_data = coops_data
                ...
                if water_level_data is not None and not water_level_data.empty:
                    ...
                    if len(merged_perf) > 0:
                        ...
                        if window_size >= 3:
                            ...
                            global_corr = merged_perf["wse_ellips_m"].corr(
            ...
            with st.spinner("Processing temporal alignment and residual analysis..."):
                ...
                if outlier_filter and len(aligned_data) > 10:
                    ...
                    outlier_mask = (aligned_data["difference"] >= lower_bound) & (
                        aligned_data["difference"] <= upper_bound
                    )
