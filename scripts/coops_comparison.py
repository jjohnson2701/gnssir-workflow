...
def find_nearest_coops_station(station_config: dict, max_distance_km: float = 50) -> dict:
    ...
    logging.info(
        f"Found nearest CO-OPS station: {nearest['id']} - {nearest['name']} "
...
def coops_comparison(
    station_name: str,
    year: int,
    doy_range: tuple = None,
    max_lag_days: int = 10,
    output_dir: Path = None,
    perform_segmented_analysis: bool = True,
) -> dict:
    """
    ...
    if coops_station_info is None:
        ...
        return {
            "success": False,
            "error": "No CO-OPS station found (checked config and auto-discovery)",
        }
    ...
    coops_daily = (
        coops_df.groupby("date").agg({"water_level_m": ["mean", "std", "count"]}).reset_index()
    )
    ...
    # Merge datasets
    merged_df = pd.merge(gnssir_df, coops_daily, on="date", how="inner")
