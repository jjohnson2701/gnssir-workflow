...
def usgs_comparison(station_name, year, doy_range=None, max_lag_days=10, output_dir=None,
    ...
    logging.info(f"Datum information: GNSS antenna ellipsoidal height = {antenna_ellipsoidal_height} m, "
    ...
    if doy_range:
        ...
        logging.info(f"Analyzing data from DOY {doy_start} to {doy_end} ({start_date} to {end_date})")
    else:
    ...
    if gnssir_daily_df is None or gnssir_daily_df.empty:
        logging.error(f"Failed to load GNSS-IR data for {station_name}, {year}, DOY range {doy_range}")
    ...
    logging.info(f"Fetching USGS data for site {usgs_site_code}, parameter {parameter_code}, dates {start_date} to {end_date}")
    ...
    if 'latitude_deg' in station_config and 'longitude_deg' in station_config:
        ...
        logging.info(f"Added GNSS coordinates to gauge_info: ({gauge_info['gnss_lat']}, {gauge_info['gnss_lon']})")
    ...
    if 'latitude' in gauge_info and 'longitude' in gauge_info:
        ...
        logging.info(f"Using USGS coordinates from gauge_info: ({gauge_info['usgs_lat']}, {gauge_info['usgs_lon']})")
    elif 'site_info' in gauge_info and 'dec_lat_va' in gauge_info['site_info'] and 'dec_long_va' in gauge_info['site_info']:
        gauge_info['usgs_lat'] = gauge_info['site_info']['dec_lat_va']
        gauge_info['usgs_lon'] = gauge_info['site_info']['dec_long_va']
        logging.info(f"Using USGS coordinates from site_info: ({gauge_info['usgs_lat']}, {gauge_info['usgs_lon']})")
    ...
    gnssir_daily_df = reflector_height_utils.calculate_wse_from_rh(gnssir_daily_df, antenna_ellipsoidal_height)
    
    # Ensure rh_count is preserved (add additional safeguard)
    if 'rh_count' in gnssir_daily_df.columns:
        logging.info(f"rh_count column found with values: Min={gnssir_daily_df['rh_count'].min()}, " 
                   f"Max={gnssir_daily_df['rh_count'].max()}, "
                   f"Mean={gnssir_daily_df['rh_count'].mean():.2f}")
    else:
        logging.warning("rh_count column not found after WSE calculation. Checking original dataframe...")
        if 'rh_count' in gnssir_daily_df.columns:
    ...
    if 'usgs_value_m_median' in usgs_daily_df.columns:
        ...
        if usgs_cols:
            ...
            return {'success': False, 'error': 'No USGS value column found for correlation analysis'}
    ...
    logging.info(f"GNSS-IR date range: {gnssir_daily_df['date'].min()} to {gnssir_daily_df['date'].max()}")
    ...
    logging.info(f"After conversion - GNSS-IR merge_date first few: {gnssir_daily_df['merge_date'].head().tolist()}")
    logging.info(f"After conversion - USGS merge_date first few: {usgs_daily_df['merge_date'].head().tolist()}")
    ...
    if 'doy' in gnssir_daily_df.columns:
        ...
        if len(gnssir_daily_df) > unique_days:
            ...
            gnssir_daily_agg = gnssir_daily_df.groupby('merge_date').agg({
                'rh_median_m': 'median',
                'wse_ellips_m': 'median',
    ...
    if len(merged_df) >= 2:  # Need at least 2 points for correlation
        ...
        rh_corr_str = f"{rh_correlation:.4f}" if rh_correlation is not None and not pd.isna(rh_correlation) else "N/A"
        wse_corr_str = f"{wse_correlation:.4f}" if wse_correlation is not None and not pd.isna(wse_correlation) else "N/A"
    ...
    if len(merged_df) >= 3:  # Need at least 3 points for lag analysis
        try:
            rh_lag, rh_lag_corr, rh_lag_conf, rh_lag_all = time_lag_analyzer.calculate_time_lag_correlation(
                merged_df['rh_median_m'],
                merged_df[usgs_value_col],
            ...
            logging.info(f"RH time lag analysis: {rh_lag} days, correlation {rh_lag_corr if rh_lag_corr is not None and not pd.isna(rh_lag_corr) else 'N/A'}, confidence {rh_lag_conf}")
        except Exception as e:
            logging.error(f"Error calculating RH time lag correlation: {e}")
        
        try:
            wse_lag, wse_lag_corr, wse_lag_conf, wse_lag_all = time_lag_analyzer.calculate_time_lag_correlation(
                merged_df['wse_ellips_m'],
                merged_df[usgs_value_col],
                max_lag_days=max_lag_days
            )
            ...
            logging.info(f"WSE time lag analysis: {wse_lag} days, correlation {wse_lag_corr if wse_lag_corr is not None and not pd.isna(wse_lag_corr) else 'N/A'}, confidence {wse_lag_conf}")
        except Exception as e:
    ...
    try:
        ...
        if 'rh_count' in gnssir_daily_df.columns:
            ...
            logging.error(f"Just before plotting: rh_count missing from columns: {gnssir_daily_df.columns.tolist()}")
    ...
    try:
        ...
        if 'rh_count' in gnssir_daily_df.columns:
            logging.info(f"Before demeaned plotting: rh_count column confirmed present in gnssir_daily_df")
        else:
            logging.error(f"Before demeaned plotting: rh_count missing! Creating a dummy column for testing")
    ...
    if wse_lag is not None and abs(wse_lag) > 0 and wse_lag_conf in ["Medium", "High"]:
        ...
        try:
            ...
            if num_points >= 3:
                ...
                lag_gnssir_df = pd.DataFrame({
                    'date': dates,
                    'wse_ellips_m': wse_series.values
                })
                
                lag_usgs_df = pd.DataFrame({
                    'date': dates,
                    usgs_value_col: usgs_series.values
                })
                ...
                logging.info(f"Generated lag-adjusted comparison plot with {num_points} points: {lag_plot_path}")
            else:
                logging.warning(f"Not enough points for lag-adjusted plot (only {num_points} points)")
                lag_plot_path = None
        except Exception as e:
    ...
    if perform_segmented_analysis and len(merged_df) >= 30:  # Only if enough data points
        try:
            ...
            if 'wse_ellips_m' in merged_df.columns:
                merged_df['wse_ellips_m_demeaned'] = merged_df['wse_ellips_m'] - merged_df['wse_ellips_m'].mean()
            if 'usgs_value_m_median' in merged_df.columns:
                merged_df['usgs_value_m_median_demeaned'] = merged_df['usgs_value_m_median'] - merged_df['usgs_value_m_median'].mean()
            
            segmented_results = perform_segmented_correlation_analysis(
                merged_df, int(year), station_name, output_dir,
...
def perform_segmented_correlation_analysis(merged_df: pd.DataFrame, 
                                         year: int, 
                                         station: str, 
                                         output_dir: Path, 
    ...
    monthly_grid_fig = visualizer.plot_segment_comparison_grid(
        monthly_data, gnss_col, usgs_col, monthly_correlations,
        max_cols=4, figsize=(18, 12),
    ...
    seasonal_grid_fig = visualizer.plot_segment_comparison_grid(
        seasonal_data, gnss_col, usgs_col, seasonal_correlations,
        max_cols=2, figsize=(14, 10),
    ...
    time_series_fig = visualizer.plot_time_series_by_segment(
        merged_df, gnss_col, usgs_col, seasonal_segments, seasonal_correlations,
    ...
    with open(summary_path, 'w') as f:
        ...
        for month, corr in monthly_correlations.items():
            if corr is not None:
                ...
                valid_count = monthly_data[month][gnss_col].notna() & monthly_data[month][usgs_col].notna()
                valid_count = valid_count.sum()
                f.write(f"{month}: {corr:.4f} ({valid_count} valid points of {point_count} total)\n")
            else:
        ...
        for season, corr in seasonal_correlations.items():
            if corr is not None:
                ...
                valid_count = seasonal_data[season][gnss_col].notna() & seasonal_data[season][usgs_col].notna()
                valid_count = valid_count.sum()
                f.write(f"{season}: {corr:.4f} ({valid_count} valid points of {point_count} total)\n")
            else:
        ...
        f.write(f"Full Period: {overall_corr:.4f} ({valid_mask.sum()} valid points of {len(merged_df)} total)\n")
    ...
    return {
        ...
        'plots': {
            'monthly_correlation': str(output_dir / f"{station}_{year}_monthly_correlation_{data_type}.png"),
            'seasonal_correlation': str(output_dir / f"{station}_{year}_seasonal_correlation_{data_type}.png"),
            'monthly_grid': str(output_dir / f"{station}_{year}_monthly_comparison_grid_{data_type}.png"),
            'seasonal_grid': str(output_dir / f"{station}_{year}_seasonal_comparison_grid_{data_type}.png"),
            'seasonal_timeseries': str(output_dir / f"{station}_{year}_seasonal_timeseries_{data_type}.png"),
            'correlation_heatmap': str(output_dir / f"{station}_{year}_correlation_heatmap_{data_type}.png")
        },
...
def main():
    ...
    parser.add_argument('--skip_segmented', action='store_true', help='Skip segmented correlation analysis')
    parser.add_argument('--generate_quality_report', action='store_true', help='Generate automated data quality report')
    ...
    if results.get('success', False):
        ...
        if correlations:
            ...
            rh_corr_str = f"{rh_corr:.4f}" if rh_corr is not None and not pd.isna(rh_corr) else "N/A"
            wse_corr_str = f"{wse_corr:.4f}" if wse_corr is not None and not pd.isna(wse_corr) else "N/A"
        ...
        if time_lag:
            ...
            wse_lag_corr_str = f"{wse_lag_corr:.4f}" if wse_lag_corr is not None and not pd.isna(wse_lag_corr) else "N/A"
            
            logging.info(f"Optimal WSE time lag: {wse_lag_days if wse_lag_days is not None else 'N/A'} days (confidence: {wse_lag_conf})")
        ...
        if 'segmented_results' in results and results['segmented_results']:
            ...
            if 'summary_file' in seg_results:
                logging.info(f"Segmented correlation analysis summary: {seg_results['summary_file']}")
        ...
        logging.error(f"Enhanced USGS comparison analysis failed: {results.get('error', 'Unknown error')}")
