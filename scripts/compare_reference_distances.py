...
def compare_station_references(station_id: str):
    ...
    if sorted_refs:
        ...
        if isinstance(current_dist, (int, float)) and best['distance_km'] < current_dist:
            ...
            print(f"  Improvement: {improvement:.1f} km closer ({current_dist:.1f} km → {best['distance_km']:.1f} km)")
            print(f"  Station ID: {best['station_id']}")
        else:
            print(f"  ✓ Current reference is optimal")
            print(f"  {current_ref['primary_source']}: {current_ref['station_name']} at {current_dist} km")
