...
def combine_daily_rh_results(station_id_4char_lower, year, daily_rh_base_dir, annual_results_dir):
    ...
    try:
        ...
        try:
            ...
            logging.info(
                f"Daily aggregated data saved to {output_csv_path} with {len(daily_agg)} total rows"
            )

            # Also save the original combined data for reference if needed
            raw_csv_path = (
                output_csv_path.parent / f"{station_id_4char_lower.upper()}_{year}_combined_raw.csv"
            )
            combined_df.to_csv(raw_csv_path, index=False)
            logging.info(
                f"Original combined data saved to {raw_csv_path} with {len(combined_df)} total rows"
            )
