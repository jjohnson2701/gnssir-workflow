...
def load_data(station: str, year: int, results_dir: Path):
    ...
    if gnssir_config_path.exists():
        with open(gnssir_config_path) as f:
            ...
            if len(azval) >= 2:
                az_ranges = [[azval[i], azval[i+1]] for i in range(0, len(azval), 2) if i+1 < len(azval)]
    ...
    print(f"  Inner reflection: {inner_reflection_dist:.1f}m from antenna, Fresnel radius: {inner_fresnel_radius:.1f}m")
    print(f"  Outer reflection: {outer_reflection_dist:.1f}m from antenna, Fresnel radius: {outer_fresnel_radius:.1f}m")
    ...
    if matched_file.exists():
        ...
        matched_df['gnss_datetime'] = pd.to_datetime(matched_df['gnss_datetime'], format='ISO8601', utc=True)
        ...
        if 'residual' not in matched_df.columns:
            ...
            ref_dm_cols = [col for col in matched_df.columns if col.endswith('_dm') and col != gnss_dm_col]
            if ref_dm_cols:
    ...
    if matched_df is not None and 'residual' in matched_df.columns:
        ...
        matched_df['MJD_approx'] = (matched_df['gnss_datetime'] - pd.Timestamp('1858-11-17')).dt.total_seconds() / 86400
...
def create_frame(df_all, df_current, df_accumulated, df_filtered_out, ref_df, metadata,
    ...
    if ref_df is not None:
        ref_window = ref_df[(ref_df['datetime'] >= start_time - timedelta(hours=6)) &
                            (ref_df['datetime'] <= end_time + timedelta(hours=6))]
        if len(ref_window) > 0 and 'wl_dm' in ref_window.columns:
            ax_ts.plot(ref_window['datetime'], ref_window['wl_dm'] * 100,
                      'r-', linewidth=2.5, alpha=0.9, label=ref_legend_label, zorder=2)

    ax_ts.scatter(df_all['datetime'], df_all['WSE_dm'] * 100,
                  c='lightgray', s=8, alpha=0.3, label='GNSS-IR (all)', zorder=1)

    if len(df_accumulated) > 0:
        ax_ts.scatter(df_accumulated['datetime'], df_accumulated['WSE_dm'] * 100,
                      c='steelblue', s=18, alpha=0.7, label='GNSS-IR (processed)', zorder=3)

    if len(df_current) > 0:
        ax_ts.scatter(df_current['datetime'], df_current['WSE_dm'] * 100,
                      c='gold', s=80, alpha=0.9, label='Current (passed)',
    ...
    if len(df_filtered_out) > 0:
        ax_ts.scatter(df_filtered_out['datetime'], df_filtered_out['WSE_dm'] * 100,
                      c='red', s=60, alpha=0.7, marker='x',
    ...
    ax_ts.set_title(f'{station_name} Water Level: GNSS-IR vs {ref_label}\n'
    ...
    for state, (lon, lat) in state_labels.items():
        ...
        if ec_x_min < sx < ec_x_max and ec_y_min < sy < ec_y_max:
            ax_coast.text(sx, sy, state, fontsize=9, ha='center', va='center',
                         color='#333333', fontweight='bold', alpha=0.7)

    # Mark station and gauge locations
    ax_coast.plot(station_x, station_y, 'r^', markersize=14, markeredgecolor='white',
                  markeredgewidth=2, zorder=10, label='GNSS Station')
    ax_coast.plot(gauge_x, gauge_y, 'bs', markersize=12, markeredgecolor='white',
                  markeredgewidth=2, zorder=10, label='Reference')

    # Draw connection line between station and gauge
    ax_coast.plot([station_x, gauge_x], [station_y, gauge_y], 'gray',
                  linestyle='--', linewidth=1.5, alpha=0.6, zorder=9)
    ...
    box_x = [station_x - buffer_wide, station_x + buffer_wide,
             station_x + buffer_wide, station_x - buffer_wide, station_x - buffer_wide]
    box_y = [station_y - buffer_wide, station_y - buffer_wide,
             station_y + buffer_wide, station_y + buffer_wide, station_y - buffer_wide]
    ...
    for az_start, az_end in az_ranges:
        ...
        wedge = Wedge((station_x, station_y), fresnel_indicator_radius, theta1, theta2,
                      facecolor='cyan', edgecolor='blue', alpha=0.35, linewidth=1.5)
        ax_map.add_patch(wedge)

    # Station and gauge markers
    ax_map.plot(station_x, station_y, 'r^', markersize=12, markeredgecolor='white',
                markeredgewidth=1.5, alpha=0.8, zorder=10, label='GNSS Station')
    ...
    ax_map.plot(gauge_x, gauge_y, 'bs', markersize=10, markeredgecolor='white',
                markeredgewidth=1.5, alpha=0.8, zorder=10, label=gauge_label)
    ax_map.plot([station_x, gauge_x], [station_y, gauge_y], 'w-', linewidth=1.5, alpha=0.5)

    # Draw box showing Fresnel zone extent
    buffer_close = 60
    box_x2 = [station_x - buffer_close, station_x + buffer_close,
              station_x + buffer_close, station_x - buffer_close, station_x - buffer_close]
    box_y2 = [station_y - buffer_close, station_y - buffer_close,
              station_y + buffer_close, station_y + buffer_close, station_y - buffer_close]
    ax_map.plot(box_x2, box_y2, 'cyan', linewidth=2, zorder=9)

    ax_map.annotate('N', (center_x, center_y + buffer_wide*0.85), ha='center',
    ...
    for az_start, az_end in az_ranges:
        ...
        inner_annulus_inner = Wedge((station_x, station_y), inner_refl_dist - inner_fresnel_r, theta1, theta2,
                                    facecolor='none', edgecolor='yellow', alpha=0.6, linewidth=1.5, linestyle='--')
        inner_annulus_outer = Wedge((station_x, station_y), inner_refl_dist + inner_fresnel_r, theta1, theta2,
                                    facecolor='none', edgecolor='yellow', alpha=0.6, linewidth=1.5, linestyle='--')
        ...
        outer_annulus_inner = Wedge((station_x, station_y), outer_refl_dist - outer_fresnel_r, theta1, theta2,
                                    facecolor='none', edgecolor='cyan', alpha=0.6, linewidth=2, linestyle='-')
        outer_annulus_outer = Wedge((station_x, station_y), outer_refl_dist + outer_fresnel_r, theta1, theta2,
                                    facecolor='none', edgecolor='cyan', alpha=0.6, linewidth=2, linestyle='-')
        ax_sat.add_patch(outer_annulus_inner)
        ax_sat.add_patch(outer_annulus_outer)

    # Station marker
    ax_sat.plot(station_x, station_y, 'r^', markersize=12, markeredgecolor='white',
    ...
    if len(df_accumulated) > 0:
        for _, row in df_accumulated.iterrows():
            ...
            ax_sat.plot(station_x + dx, station_y + dy, 'o', markersize=4,
                       color=color, alpha=0.5, markeredgecolor='none')
    ...
    if len(df_current) > 0:
        for _, row in df_current.iterrows():
            ...
            color = cmap(norm(row['WSE_dm'] * 100))
            ax_sat.plot(station_x + dx, station_y + dy, 'o', markersize=12,
                       color=color, alpha=0.95, markeredgecolor='darkorange',
    ...
    ax_sat.annotate('N', (station_x, station_y + compass_offset), ha='center', fontsize=11,
                    fontweight='bold', color='white')
    ax_sat.annotate('E', (station_x + compass_offset, station_y), ha='center', fontsize=11,
                    fontweight='bold', color='white')
...
def create_animation(station: str, year: int, doy_start: int, doy_end: int,
    ...
    if ref_df is not None and 'wl_dm' in ref_df.columns:
        ref_window = ref_df[(ref_df['datetime'] >= start_time - timedelta(days=1)) &
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i, bin_end in enumerate(bin_times):
            ...
            df_current_all = df_all_unfiltered[(df_all_unfiltered['datetime'] >= bin_start) &
...
def main():
    ...
    output_path = results_dir / args.station / f"{args.station}_{args.year}_polar_animation_DOY{args.doy_start}-{args.doy_end}.gif"
