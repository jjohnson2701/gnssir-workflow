...
def plot_segment_correlations(
    ...
    filtered_correlations = {
        k: v for k, v in segment_correlations.items() if v is not None and not pd.isna(v)
    }

    if not filtered_correlations:
        fig, ax = plt.subplots(figsize=(8, 6))
        ax.text(
            0.5, 0.5, "No valid correlation data available", ha="center", va="center", fontsize=14
        )
        ax.axis("off")
        return fig
...
def plot_segment_comparison_grid(
    ...
    if n_segments == 0:
        fig, ax = plt.subplots(figsize=(8, 6))
        ax.text(0.5, 0.5, "No valid segment data available", ha="center", va="center", fontsize=14)
        ax.axis("off")
        return fig
...
def plot_time_series_by_segment(
    df: pd.DataFrame,
    gnss_col: str,
    usgs_col: str,
    ...
    for i, (segment_name, date_criteria) in enumerate(segments_dict.items()):
        ...
        segment_df = segment_df.loc[
            df_copy.index.isin(filter_by_segment(df_copy, date_criteria).index)
        ]
    ...
    # Format axes
    ax1.set_title(
        f"{station_name} GNSS-IR vs USGS Segmented Analysis" + (" (Demeaned)" if demeaned else "")
    )
...
def plot_heatmap_correlation_matrix(
    segment_correlations: Dict[str, float],
    ...
    if n_segments <= 5:
        corr_df = pd.DataFrame(
            corr_series.values.reshape(1, -1), index=["Correlation"], columns=corr_series.index
        )
    else:
        ...
        corr_df = pd.DataFrame(
            padded_values.reshape(n_rows, n_cols),
            index=[f"Row {i+1}" for i in range(n_rows)],
            columns=[corr_series.index[i] if i < len(corr_series) else "" for i in range(n_cols)],
        )
    ...
    # Create heatmap
    sns.heatmap(
        corr_df, annot=True, cmap=cmap, vmin=-1, vmax=1, linewidths=0.5, ax=ax, fmt=".2f", center=0
    )
