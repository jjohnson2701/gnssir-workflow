...
def calculate_optimal_time_lag(
    ...
    try:
        ...
        if 'date' in gnssir_df.columns:
            ...
            logging.error("Cannot calculate time lag: date/datetime column not found in GNSS-IR DataFrame")
            return result
        ...
        if 'date' in usgs_df.columns:
            ...
            logging.error("Cannot calculate time lag: date/datetime column not found in USGS DataFrame")
            return result
        ...
        max_overlap = max([sum((~gnss_reindexed.shift(lag).isna()) & (~usgs_reindexed.isna())) 
                         for lag in range(-max_lag_days, max_lag_days + 1)])
        ...
        lag_is_significant = (best_corr > 0.5 and (best_lag != 0) and (best_corr - zero_lag_corr > 0.05))
        ...
        if lag_is_significant or best_lag != 0:
            ...
            lag_adjusted_gnssir = pd.DataFrame({
                'date': lag_adjusted_dates,
                gnssir_col: lag_adjusted_values
            })
...
def plot_lag_correlation(
    ...
    try:
        ...
        plt.title(f"Time Lag Analysis: {station_name} GNSS-IR vs. USGS {usgs_site_code}", fontsize=16)
...
def plot_lag_adjusted_comparison(
    ...
    try:
        ...
        if not lag_results.get('lag_significant', False) and lag_results.get('optimal_lag_days', 0) == 0:
        ...
        if 'datetime' in daily_gnssir_rh_df.columns and 'datetime' not in adjusted_gnssir_df.columns:
