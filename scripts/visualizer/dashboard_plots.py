...
def create_calendar_heatmap(
    ...
    """
    Create a calendar heat map showing daily performance metrics.
    ...
    for month in range(1, 13):
        ...
        for week_num, week in enumerate(cal):
            for day_num, day in enumerate(week):
                ...
                # Draw rectangle
                rect = Rectangle((day_num, 4 - week_num), 1, 1, 
                               facecolor=color, edgecolor='black', 
                               linewidth=0.5, alpha=alpha)
                ax.add_patch(rect)
                
                # Add day number
                ax.text(day_num + 0.5, 4.5 - week_num, str(day),
    ...
    # Set main title
    if title is None:
        title = f"{station_name} {year} - Daily {metric_col.replace('_', ' ').title()}"
...
def create_monthly_box_plots(
    ...
    # Create box plot
    box_data = [plot_df[plot_df['month'] == m][metric_col].dropna() 
                for m in range(1, 13)]
    
    bp = ax.boxplot(box_data, positions=range(1, 13), widths=0.6,
...
def create_multi_parameter_timeline(
    ...
    if 'rh_count' in gnssir_df.columns:
        ...
        ax1.bar(gnssir_df['date'], gnssir_df['rh_count'], 
                color=colors['gnss_smooth'], alpha=0.6, label='Daily Retrievals')
        
        # Rolling average
        rolling_avg = gnssir_df.set_index('date')['rh_count'].rolling(
            window=rolling_window, center=True).mean()
        ax1.plot(rolling_avg.index, rolling_avg.values, 
                color='red', linewidth=2, label=f'{rolling_window}-day Average')
    ...
    if 'rh_std_m' in gnssir_df.columns:
        ...
        ax2.fill_between(gnssir_df['date'], 0, gnssir_df['rh_std_m'],
                        color=colors['usgs'], alpha=0.5, label='Daily Std Dev')
        
        # Rolling statistics
        rolling_std = gnssir_df.set_index('date')['rh_std_m'].rolling(
            window=rolling_window, center=True).mean()
        ax2.plot(rolling_std.index, rolling_std.values,
                color='darkred', linewidth=2, label=f'{rolling_window}-day Avg Std')
    ...
    if usgs_df is not None and 'date' in usgs_df.columns and 'rh_median_m' in gnssir_df.columns:
        ...
        if usgs_value_col:
            ...
            merged = pd.merge(gnssir_df[['date', gnss_col]], 
            ...
            if len(merged) > rolling_window:
                ...
                rolling_corr = merged[gnss_col].rolling(window=rolling_window).corr(
                    merged['water_level'])
                
                # Determine data source label
                source_label = "Water Level"
                
                ax3.plot(rolling_corr.index, rolling_corr.values,
                        color=colors['correlation'], linewidth=2.5,
                ...
                ax3.text(0.5, 0.5, f'Insufficient data for correlation\n(need >{rolling_window} days)', 
                        ha='center', va='center', transform=ax3.transAxes, fontsize=12)
                ax3.set_ylabel('Correlation', fontsize=12)
                ax3.grid(True, alpha=0.3)
    else:
        ax3.text(0.5, 0.5, 'No water level data available\nfor correlation analysis', 
                ha='center', va='center', transform=ax3.transAxes, fontsize=12)
        ax3.set_ylabel('Correlation', fontsize=12)
        ax3.grid(True, alpha=0.3)
    ...
    if environmental_df is not None and n_panels > 3:
        ...
        if 'wind_speed' in env_df.columns:
            ax4.plot(env_df['date'], env_df['wind_speed'],
                    color=colors['ndbc'], linewidth=1.5, alpha=0.7,
                    label='Wind Speed (m/s)')
        
        if 'wave_height' in env_df.columns:
            ax4_twin = ax4.twinx()
            ax4_twin.plot(env_df['date'], env_df['wave_height'],
                         color=colors['coops'], linewidth=1.5, alpha=0.7,
                         label='Wave Height (m)')
...
def classify_tidal_stage(
    ...
    rate = calculate_water_level_change_rate(
        df, value_col='level', time_col='time',
...
def create_calendar_heatmap(
    ...
    if metric_col == 'correlation':
        ...
        cbar.ax.text(0.02, -1.2, 'Poor', transform=cbar.ax.transAxes, 
                    ha='left', va='top', fontsize=12, color='darkred', fontweight='bold')
        cbar.ax.text(0.98, -1.2, 'Excellent', transform=cbar.ax.transAxes, 
                    ha='right', va='top', fontsize=12, color='darkgreen', fontweight='bold')
    elif metric_col == 'rmse':
        cbar.ax.text(0.02, -1.2, 'Accurate', transform=cbar.ax.transAxes, 
                    ha='left', va='top', fontsize=12, color='darkgreen', fontweight='bold')
        cbar.ax.text(0.98, -1.2, 'Error-prone', transform=cbar.ax.transAxes, 
                    ha='right', va='top', fontsize=12, color='darkred', fontweight='bold')
    elif metric_col == 'rh_count':
        # Make measurement count labels larger and more prominent
        cbar.ax.text(0.02, -1.2, 'FEW\nMeasurements', transform=cbar.ax.transAxes, 
                    ha='left', va='top', fontsize=14, color='darkred', fontweight='bold',
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor='darkred', alpha=0.8))
        cbar.ax.text(0.98, -1.2, 'MANY\nMeasurements', transform=cbar.ax.transAxes, 
                    ha='right', va='top', fontsize=14, color='darkgreen', fontweight='bold',
    ...
    fig.text(0.02, 0.03, f'Data Coverage: {available_days}/{total_days} days ({coverage_pct:.1f}%)', 
...
def create_tidal_stage_performance(
    ...
    gnssir_df['datetime'] = pd.to_datetime(gnssir_df['datetime'] if 'datetime' in gnssir_df.columns else gnssir_df['date'])
    tide_df['datetime'] = pd.to_datetime(tide_df['datetime'] if 'datetime' in tide_df.columns else tide_df['date'])
    ...
    if tide_col is None:
        ...
        for ax in axes:
            ax.text(0.5, 0.5, 'No tide data available', ha='center', va='center', transform=ax.transAxes)
            ax.set_xticks([])
            ax.set_yticks([])
        return fig
    ...
    if len(tide_df) > 7:
        ...
        sample_data = tide_df.iloc[sample_start:sample_start + min(168, len(tide_df) - sample_start)]  # Up to 7 days
        
        # Plot water level
        ax_timeline.plot(sample_data['datetime'], sample_data[tide_col], 
                        color=colors['usgs'], linewidth=2, label='Water Level')
        
        # Color-code by tidal stage
        for stage in ['High', 'Low', 'Rising', 'Falling']:
            stage_data = sample_data[sample_data['tidal_stage'] == stage]
            if len(stage_data) > 0:
                ax_timeline.scatter(stage_data['datetime'], stage_data[tide_col],
                                  c=stage_colors[stage], label=stage, alpha=0.8, s=20)
        
        ax_timeline.set_ylabel('Water Level (m)', fontsize=12)
        ax_timeline.set_xlabel('Date/Time', fontsize=12)
        ax_timeline.set_title(f'Sample Weekly Timeline: Water Level and Tidal Stages\n'
        ...
        legend = ax_timeline.legend(bbox_to_anchor=(1.02, 1), loc='upper left', 
    ...
    for stage in ['Low', 'Rising', 'High', 'Falling']:
        stage_subset = merged_df[merged_df['tidal_stage'] == stage]
        if len(stage_subset) > 0:
            ax3.scatter(stage_subset['tide_level'], stage_subset['rh_std_m'],
                       c=stage_colors[stage], label=stage, alpha=0.6, s=30)
    ...
    for stage in ['Low', 'Rising', 'High', 'Falling']:
        if stage in hourly_pcts.columns:
            ax4.bar(range(24), hourly_pcts[stage], bottom=bottom,
                   color=stage_colors[stage], alpha=0.7, label=stage)
    ...
    if title is None:
        title = f"{station_name} - Tidal Stage Performance Analysis\n" + \
...
def load_sub_hourly_data(station_name: str, year: int, sample_days: int = 30) -> Optional[pd.DataFrame]:
    """
...
def create_multi_scale_performance(
    ...
    if try_load_real_data and station_name != "Test Station":
        ...
        if not daily_df.empty and 'date' in daily_df.columns:
            ...
            if real_sub_hourly_data is not None and not real_sub_hourly_data.empty:
                ...
                data_source_note = f"Using actual sub-hourly data ({len(sub_hourly_df)} observations)"
                logger.info(f"Successfully loaded real sub-hourly data: {len(sub_hourly_df)} points")
    ...
    if 'datetime' in sub_hourly_df.columns:
        ...
        if hasattr(sub_hourly_df.index, 'to_pydatetime'):
            ...
            sub_hourly_df['datetime'] = pd.date_range(start='2024-01-01', periods=len(sub_hourly_df), freq='H')
    ...
    sub_hourly_daily_stats.columns = ['_'.join(col).strip() if isinstance(col, tuple) else col 
    ...
    if 'rh_count' in comparison_df.columns:
        ...
        density_bins = pd.qcut(comparison_df['retrieval_density'], q=4, 
        ...
        density_bins = pd.cut(comparison_df['retrieval_density'], bins=4, 
                             labels=['Low', 'Medium', 'High', 'Very High'])
    ...
    for i in range(2):
        for j in range(len(density_labels)):
            if not np.isnan(precision_array[i, j]):
                ax1.text(j, i, f'{precision_array[i, j]:.3f}', 
    ...
    if environmental_df is not None and not environmental_df.empty:
        ...
        if wind_col is not None:
            env_comparison = pd.merge(comparison_df, env_df[['date', wind_col]], on='date', how='inner')
            
            if len(env_comparison) > 10:  # Need sufficient data
                # Bin by wind conditions
                try:
                    wind_bins = pd.qcut(env_comparison[wind_col], q=3, 
                    ...
                    # Remove NaN values
                    valid_data = [(label, val) for label, val in zip(wind_labels, wind_precision) if not np.isnan(val)]
                    
                    if valid_data:
                        labels, values = zip(*valid_data)
                        bars = ax2.bar(labels, values, color=[colors['ndbc'], colors['coops'], colors['usgs']][:len(labels)], alpha=0.7)
                        ax2.set_ylabel('RH Standard Deviation (m)')
                        ax2.set_title('Precision vs Wind Conditions', fontsize=12, fontweight='bold')
    ...
    ax4.plot(comparison_df['date'], comparison_df['rh_std_m'], 
             color=colors['gnss_smooth'], linewidth=2, label='Daily Aggregated', alpha=0.8)
    ax4.plot(comparison_df['date'], comparison_df['rh_std'], 
             color=colors['usgs'], linewidth=1.5, label='Sub-hourly Derived', alpha=0.8)
    ...
    if window >= 3:
        ...
        ax4.plot(comparison_df['date'], daily_smooth, 
                color=colors['gnss_smooth'], linewidth=3, alpha=0.5, linestyle='--')
        ax4.plot(comparison_df['date'], subhourly_smooth, 
                color=colors['usgs'], linewidth=3, alpha=0.5, linestyle='--')
    ...
    ax5.bar(x - width/2, daily_metrics, width, label='Daily', 
           color=colors['gnss_smooth'], alpha=0.7)
    ax5.bar(x + width/2, subhourly_metrics, width, label='Sub-hourly', 
           color=colors['usgs'], alpha=0.7)
    ...
    if 'rh_std' in comparison_df.columns and comparison_df['rh_std'].notna().sum() > 5:
        ...
        if len(valid_data) > 5:
            ...
            ax6.scatter(valid_data['rh_std_m'], valid_data['rh_std'], 
                       alpha=0.6, color=colors['correlation'], s=30)
            ...
            p = np.poly1d(z)
            ax6.plot(valid_data['rh_std_m'].sort_values(), p(valid_data['rh_std_m'].sort_values()), 
        ...
        if 'rh_count' in comparison_df.columns:
            scatter = ax6.scatter(comparison_df['rh_count'], comparison_df['rh_std_m'], 
                                 alpha=0.6, color=colors['correlation'], s=30)
            
            # Add trend line
            z = np.polyfit(comparison_df['rh_count'], comparison_df['rh_std_m'], 1)
            p = np.poly1d(z)
            ax6.plot(comparison_df['rh_count'].sort_values(), p(comparison_df['rh_count'].sort_values()), 
            ...
            ax6.set_title(f'Data Density vs Precision (r={corr_coef:.3f})', fontsize=12, fontweight='bold')
        else:
            ax6.text(0.5, 0.5, 'Scale correlation\ndata not available', 
                    ha='center', va='center', transform=ax6.transAxes, fontsize=12)
    ...
    if 'rh_count' in comparison_df.columns:
        ax7.hist(comparison_df['rh_count'], bins=15, color=colors['gnss_smooth'], 
                alpha=0.7, edgecolor='black', linewidth=0.5)
        ax7.axvline(comparison_df['rh_count'].mean(), color='red', 
                   linestyle='--', linewidth=2, label=f'Mean: {comparison_df["rh_count"].mean():.1f}')
        ax7.set_xlabel('Daily Retrieval Count')
        ax7.set_ylabel('Frequency')
        ax7.legend()
    else:
        # Show retrieval density instead
        ax7.hist(comparison_df['retrieval_density'], bins=15, color=colors['gnss_smooth'], 
                alpha=0.7, edgecolor='black', linewidth=0.5)
    ...
    if title is None:
        title = f"{station_name} - Multi-Scale Performance Matrix\n" + \
...
def create_water_level_change_response(
    ...
    # Ensure datetime columns
    gnssir_df['datetime'] = pd.to_datetime(gnssir_df['datetime'] if 'datetime' in gnssir_df.columns else gnssir_df['date'])
    usgs_df['datetime'] = pd.to_datetime(usgs_df['datetime'] if 'datetime' in usgs_df.columns else usgs_df['date'])
    ...
    if water_level_col is None:
        ...
        for ax in axes:
            ax.text(0.5, 0.5, 'No USGS water level data', ha='center', va='center', transform=ax.transAxes)
            ax.set_xticks([])
            ax.set_yticks([])
        return fig
    ...
    if len(merged_df) == 0:
        ...
        for ax in axes:
            ax.text(0.5, 0.5, 'No valid change rate data', ha='center', va='center', transform=ax.transAxes)
            ax.set_xticks([])
            ax.set_yticks([])
        return fig
    ...
    rate_bins = pd.qcut(abs_change_rate, q=4, labels=['Slow', 'Moderate', 'Fast', 'Very Fast'],
                       duplicates='drop')
    ...
    scatter = ax2.scatter(abs_change_rate, merged_df['rh_std_m'], 
    ...
    if len(merged_df) > 10:
        ...
        p = np.poly1d(z)
        ax2.plot(sorted(abs_change_rate), p(sorted(abs_change_rate)), 
                "r--", alpha=0.8, linewidth=2)
    ...
    if rate_count_data:
        ...
        ax3.text(0.5, 0.5, 'Retrieval count data\nnot available', ha='center', va='center', 
    ...
    line1 = ax4.plot(plot_df['datetime'], plot_df['rh_std_m'], 
                     color=colors['gnss_smooth'], linewidth=2, label='RH Precision')
    line2 = ax4_twin.plot(plot_df['datetime'], np.abs(plot_df['change_rate']), 
                         color=colors['usgs'], linewidth=1.5, alpha=0.7, label='|Change Rate|')
    ...
    if title is None:
        title = f"{station_name} - Water Level Change Rate Response\n" + \
