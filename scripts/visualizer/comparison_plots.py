...
def create_comparison_plot(df, station_name, year, output_path):
    ...
    fig.suptitle(
        f"{station_name} GNSS-IR vs USGS Water Level Comparison ({year})",
        fontsize=16,
        fontweight="bold",
        y=0.95,
    )
    ...
    ax2.text(
        0.02,
        0.98,
        f"Demeaned Correlation: {demeaned_corr:.3f}",
...
def create_quality_diagnostic_plot(df, station_name, year, output_path):
    ...
    fig.suptitle(
        f"{station_name} Data Quality Diagnostics ({year})", fontsize=16, fontweight="bold"
    )
    ...
    axes[0, 0].bar(
        months, availability["wse_ellips_m"], alpha=0.7, label="GNSS-IR", color=PLOT_COLORS["gnss"]
    )
    axes[0, 0].bar(
        months,
        availability["usgs_value_m_median"],
        alpha=0.7,
        label="USGS",
    ...
    if len(valid_data) > 0:
        ...
        for season in ["Winter", "Spring", "Summer", "Fall"]:
            ...
            if mask.any():
                axes[1, 0].scatter(
                    valid_data.loc[mask, "wse_ellips_m"],
        ...
        x_line = np.linspace(
            valid_data["wse_ellips_m"].min(), valid_data["wse_ellips_m"].max(), 100
        )
    ...
    if "rh_std_m" in df.columns and len(valid_data) > 0:
        ...
        if len(quality_data) > 0:
            axes[1, 1].scatter(
                quality_data,
                residuals.loc[quality_data.index],
                alpha=0.6,
                s=50,
                color=PLOT_COLORS["correlation"],
            )
...
def create_spring_investigation_plot(df, station_name, year, output_path):
    ...
    fig.suptitle(
        f"{station_name} Spring Data Investigation ({year})", fontsize=16, fontweight="bold"
    )

    # 1. Time series comparison
    axes[0, 0].plot(
        valid_spring.index,
        valid_spring["wse_ellips_m"],
        "b-o",
        label="GNSS-IR WSE",
        markersize=6,
        color=PLOT_COLORS["gnss"],
    )
    ax_twin = axes[0, 0].twinx()
    ax_twin.plot(
        valid_spring.index,
        valid_spring["usgs_value_m_median"],
        "r-s",
        label="USGS WL",
        markersize=4,
        color=PLOT_COLORS["usgs"],
    ...
    if len(valid_spring) > 2:
        ...
        x_line = np.linspace(
            valid_spring["wse_ellips_m"].min(), valid_spring["wse_ellips_m"].max(), 100
        )
        y_line = slope * x_line + intercept
    ...
    axes[1, 0].plot(
        valid_spring.index, residuals, "go-", markersize=6, color=PLOT_COLORS["correlation"]
    )
    ...
    if "rh_count" in valid_spring.columns:
        axes[1, 1].scatter(
            valid_spring["rh_count"], residuals, alpha=0.7, s=80, color=PLOT_COLORS["correlation"]
        )
