...
def find_column_by_name(
    ...
    if column_position is not None and len(df.columns) > column_position:
        ...
        logging.info(f"Using column at position {column_position+1} as {column_type} column: {col_name}")
        return col_name
...
def add_summary_textbox(
    ...
    try:
        # Default box style
        default_style = dict(facecolor='white', alpha=0.8, boxstyle='round,pad=0.5', edgecolor='gray')
        ...
        plt.figtext(position[0], position[1], summary_text, fontsize=fontsize,
                  bbox=default_style, horizontalalignment='left', verticalalignment='bottom')
