...
def get_noaa_tide_predictions(
    ...
    """
    logging.info(f"Fetching NOAA tide predictions for station {station_id} from {start_date} to {end_date}")
...
def generate_synthetic_tide_predictions(
    ...
    try:
        ...
        tide_level = (
            mean_level +
            m2 + s2 +                                 # Main semidiurnal constituents
    ...
    try:
        ...
        for i, obs_time in enumerate(observed_times):
            try:
                ...
                closest_time = resampled_tide.index[resampled_tide.index.get_indexer([obs_time], method='nearest')[0]]
        ...
        if mean_offset is None:
            ...
            valid_mask = np.isfinite(obs_df[observed_wl_col]) & np.isfinite(obs_df['tide_prediction_m'])
            if valid_mask.sum() > 0:
                mean_offset = np.mean(obs_df.loc[valid_mask, observed_wl_col] - obs_df.loc[valid_mask, 'tide_prediction_m'])
            else:
                mean_offset = 0.0
...
def plot_subdaily_rh_vs_tide(
    ...
    try:
        ...
        tide_station_name = tide_station_info.get('name', f'Station {tide_station_id}') if tide_station_info else f'Tide Station'
        tide_datum = tide_station_info.get('reference_datum', 'Unknown') if tide_station_info else 'Unknown'
        
        usgs_site_code = usgs_gauge_info.get('site_code', 'Unknown') if usgs_gauge_info else 'Unknown'
        usgs_site_name = usgs_gauge_info.get('site_name', f'USGS {usgs_site_code}') if usgs_gauge_info else 'USGS Gauge'
        usgs_vertical_datum = usgs_gauge_info.get('vertical_datum', 'Unknown') if usgs_gauge_info else 'Unknown'
        ...
        gs = gridspec.GridSpec(n_panels, 1, height_ratios=[3, 1] if n_panels == 2 else [1], hspace=0.15)
        
        # Panel 1: Sub-daily RH and Tide
        ax1 = fig.add_subplot(gs[0])
        
        # Plot tide predictions as continuous line
        ax1.plot(tide_df[tide_datetime_col], tide_df[tide_wl_col], '-',
        ...
        if has_usgs_data:
            ax1.plot(usgs_iv_df[usgs_datetime_col], usgs_iv_df[usgs_wl_col], 's-',
