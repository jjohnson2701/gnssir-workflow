...
from .base import ensure_output_dir, find_column_by_name, add_summary_textbox, PLOT_COLORS, PLOT_STYLES
...
def plot_annual_rh_timeseries(
    ...
    try:
        ...
        # Plot RH data
        plt.scatter(x_vals, df[rh_column], 
                  label=f"{station_name} Reflector Height", 
                  alpha=0.7, s=15, color=PLOT_COLORS['gnssir'])
        ...
        if reference_gauge_data_path:
            try:
                ...
                ref_time_col = [col for col in ref_df.columns if any(
                    time_str in col.lower() for time_str in ['time', 'date']
                )][0]
                
                ref_level_col = [col for col in ref_df.columns if any(
                ...
                # Plot reference data
                plt.plot(ref_df[ref_time_col], ref_df[ref_level_col], 
        ...
        if highlight_dates and date_present:
            try:
                ...
                for date in highlight_dates_dt:
                    ...
                    plt.scatter(x_vals.iloc[closest_idx], df[rh_column].iloc[closest_idx], 
                              color=PLOT_COLORS['highlight'], s=100, zorder=10, 
                              label=f"Highlight: {date.strftime('%Y-%m-%d')}" if date == highlight_dates_dt[0] else "")
