...
def plot_ribbon_comparison(
    ...
    try:
        ...
        has_gnssir_range = (
            gnssir_min_col in gnssir_df.columns and gnssir_max_col in gnssir_df.columns
        )
        ...
        ax.plot(
            ...
            color="#C0392B",
            linewidth=2,
            label=f"USGS {usgs_site_code} median",
            zorder=3,
        )
        ...
        ax.plot(
            ...
            color="#2471A3",
            linewidth=2,
            label=f"{station_name} GNSS-IR median",
            zorder=4,
        )
        ...
        ax.set_title(
            f"{station_name} vs USGS {usgs_site_code} Water Level{title_suffix}", fontsize=14
        )
        ...
        if usgs_site_name:
            display_name = (
                usgs_site_name[:25] + "..." if len(usgs_site_name) > 25 else usgs_site_name
            )
...
def plot_comparison_timeseries(
    ...
    try:
        ...
        if calc_correlation is None:
            try:
                if "date" in daily_gnssir_rh_df.columns and "date" in daily_usgs_gauge_df.columns:
                    if (
                        gnssir_rh_col in daily_gnssir_rh_df.columns
                        and usgs_wl_col in daily_usgs_gauge_df.columns
                    ):
                        ...
                        gnssir_for_merge["merge_date"] = pd.to_datetime(
                            gnssir_for_merge["date"]
                        ).dt.strftime("%Y-%m-%d")
                        usgs_for_merge["merge_date"] = pd.to_datetime(
                            usgs_for_merge["date"]
                        ).dt.strftime("%Y-%m-%d")
                        ...
                        if merged_df is not None and len(merged_df) >= 2:
                            ...
                            calc_rmse = calculate_rmse(
                                merged_df[gnssir_rh_col], merged_df[usgs_wl_col]
                            )
        ...
        gnssir_x = (
            daily_gnssir_rh_df["datetime"]
            if "datetime" in daily_gnssir_rh_df.columns
            else daily_gnssir_rh_df["date"]
        )
        usgs_x = (
            daily_usgs_gauge_df["datetime"]
            if "datetime" in daily_usgs_gauge_df.columns
            else daily_usgs_gauge_df["date"]
        )
        ...
        if compare_demeaned:
            _create_demeaned_comparison_plot(
                ...
                usgs_x,
                daily_usgs_gauge_df[usgs_wl_col],
                station_name,
                usgs_site_code,
                highlight_dates,
...
def _create_demeaned_comparison_plot(
    ...
    gnssir_df = pd.DataFrame(
        {"x": gnssir_x.reset_index(drop=True), "y": gnssir_y.reset_index(drop=True)}
    )
    ...
    if count_values is not None and len(count_values) == len(gnssir_df):
        ...
        logging.info(
            f"Using provided count values: Min={min(count_values)}, Max={max(count_values)}"
        )
    else:
    ...
    ax.plot(gnssir_df["x"], gnssir_df["y"], "-", color=PLOT_COLORS["gnssir"], alpha=0.3, zorder=5)
        ...
        try:
            ...
            for date in highlight_dates_dt:
                ...
                if len(gnssir_x) > 0:
                    ...
                    ax.scatter(
                        gnssir_x.iloc[closest_gnssir_idx],
                        gnssir_y.iloc[closest_gnssir_idx],
                ...
                if len(usgs_x) > 0:
                    ...
                    ax.scatter(
                        usgs_x.iloc[closest_usgs_idx],
                        usgs_y.iloc[closest_usgs_idx],
                        color=PLOT_COLORS["highlight"],
                        s=100,
                        zorder=10,
                    )
...
def _create_dual_axis_comparison_plot(
    ...
    # Add a light connecting line
    ax1.plot(gnssir_df["x"], gnssir_df["y"], "-", color=PLOT_COLORS["gnssir"], alpha=0.3, zorder=5)
    ...
    ax2.set_ylabel(
        f"{usgs_site_code} USGS Water Level (m)",
        fontsize=14,
        color=PLOT_COLORS["usgs"],
        labelpad=20,
    )
    ...
    try:
        ...
        if "UTCtime" in gnssir_df.columns and "doy" in gnssir_df.columns:
            ...
            gnssir_df["datetime"] = pd.to_datetime(
                gnssir_df["year"].astype(str) + "-" + gnssir_df["doy"].astype(str), format="%Y-%j"
        ...
        if wl_col is None:
            logging.error(
                f"No water level column found in USGS data. Columns: {usgs_df.columns.tolist()}"
            )
            return output_paths

        usgs_df["usgs_wl_m"] = pd.to_numeric(usgs_df[wl_col], errors="coerce") * 0.3048  # ft to m
        usgs_df = (
            usgs_df.dropna(subset=["usgs_wl_m"]).sort_values("datetime").reset_index(drop=True)
        )
        ...
        logging.info(
            f"Full year subdaily stats: r={correlation:.3f}, RMSE={rmse:.3f}m, N={n_points}"
        )
    ...
    try:
        ...
        for i, month in enumerate(months):
            ...
            if len(month_data) == 0 or len(usgs_month) == 0:
                ax_main.text(
                    0.5, 0.5, "No Data", ha="center", va="center", transform=ax_main.transAxes
                )
            ...
            _plot_with_gap_masking(
                ax_main,
                usgs_sorted["datetime"],
                usgs_sorted["usgs_dm"],
                gap_threshold_hours,
                color="#C0392B",
                linewidth=1.5,
            ...
            if len(month_sorted) >= ribbon_window:
                rolling_std = (
                    month_sorted["gnss_dm"].rolling(ribbon_window, center=True, min_periods=1).std()
                )
                gnss_upper = month_sorted["gnss_dm"] + rolling_std
                gnss_lower = month_sorted["gnss_dm"] - rolling_std

                # Mask ribbon gaps
                _fill_between_with_gaps(
                    ax_main,
                    month_sorted["gnss_datetime"].values,
                    gnss_lower.values,
                    gnss_upper.values,
                    gap_threshold_hours,
                    alpha=0.3,
                    color="#3498DB",
                    label="GNSS-IR scatter (±σ)",
                    zorder=1,
                )

            # Plot GNSS-IR points
            ax_main.scatter(
                month_sorted["gnss_datetime"],
                month_sorted["gnss_dm"],
                c="#2471A3",
                s=15,
                alpha=0.7,
                zorder=3,
            ...
            # Title with stats
            ax_main.set_title(
                f"{month_names[i]}: r={month_corr:.2f}, RMSE={month_rmse:.2f}m, N={month_n}",
                fontsize=10,
            )
            ...
            if ax_resid is not None and len(month_data) > 0:
                ...
                ax_resid.scatter(
                    month_sorted["gnss_datetime"],
                    residuals.loc[month_sorted.index],
                    c="#8E44AD",
                    s=10,
                    alpha=0.7,
                )
        ...
        fig.legend(
            handles, labels, loc="upper center", ncol=4, fontsize=10, bbox_to_anchor=(0.5, 0.99)
        )
...
def _create_subdaily_overview(
    ...
    try:
        ...
        week_mask = (sample_data["gnss_datetime"] >= start_date) & (
            sample_data["gnss_datetime"] <= end_date
        )
        ...
        if len(week_sorted) >= ribbon_window:
            rolling_std = (
                week_sorted["gnss_dm"].rolling(ribbon_window, center=True, min_periods=1).std()
            )
            ...
            _fill_between_with_gaps(
                ax_main,
                week_sorted["gnss_datetime"].values,
                gnss_lower.values,
                gnss_upper.values,
                gap_threshold_hours,
                alpha=0.3,
        ...
        info_lines.extend(
            [
                (
                    f"USGS: {usgs_site_name[:25]}..."
                    if len(usgs_site_name) > 25
                    else f"USGS: {usgs_site_name}"
                ),
                f"Datum: {usgs_datum}",
            ]
        )
        ...
        ax_main.set_title(
            f"{station_name} Subdaily: GNSS-IR WSE vs USGS IV ({date_range})", fontsize=14
        )
        ...
        if ax_resid is not None:
            ...
            ax_resid.scatter(week_sorted["gnss_datetime"], residuals, c="#8E44AD", s=20, alpha=0.8)
            ax_resid.axhline(0, color="gray", linestyle="--", alpha=0.5)
...
def _plot_with_gap_masking(ax, x_data, y_data, gap_threshold_hours: float, **plot_kwargs) -> None:
...
def _fill_between_with_gaps(
    ax, x_data, y_lower, y_upper, gap_threshold_hours: float, **fill_kwargs
) -> None:
    ...
    for gap_idx in gap_indices:
        ...
        if first_segment:
            ax.fill_between(
                x_arr[start_idx:end_idx],
                y_low[start_idx:end_idx],
                y_up[start_idx:end_idx],
                label=label,
                **fill_kwargs,
            )
            first_segment = False
