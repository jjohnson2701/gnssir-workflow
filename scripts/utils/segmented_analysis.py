...
def filter_by_segment(df: pd.DataFrame, date_criteria: Any) -> pd.DataFrame:
    ...
    if isinstance(date_criteria, tuple) and len(date_criteria) == 2:
        if isinstance(date_criteria[0], (str, pd.Timestamp)) and isinstance(date_criteria[1], (str, pd.Timestamp)):
            # Date range: ("2024-01-01", "2024-03-31")
            start_date, end_date = pd.to_datetime(date_criteria[0]), pd.to_datetime(date_criteria[1])
...
def perform_segmented_correlation(df: pd.DataFrame, 
                                 segments_dict: Dict, 
    ...
    for segment_name, date_criteria in segments_dict.items():
        ...
        if valid_count >= min_points:
            ...
            logger.warning(f"Not enough valid data points for correlation in segment: {segment_name} ({valid_count} points)")
            segment_correlations[segment_name] = None
