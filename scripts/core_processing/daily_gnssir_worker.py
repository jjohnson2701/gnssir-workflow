...
from ..external_tools.gnssrefl_executor import (
    execute_rinex2snr,
    execute_gnssir,
...
def process_single_day(
    ...
    if json_target_path is None:
        day_logger.error(
            f"Failed to copy JSON parameter file for {station_id.upper()} {year} {doy_padded}"
        )
    ...
    if skip_options.get("skip_snr", False):
        ...
        if check_file_exists(snr_file_path, min_size_bytes=MIN_SNR_SIZE_BYTES) or check_file_exists(
            snr_file_path_gz, min_size_bytes=MIN_SNR_SIZE_BYTES
        ):
            day_logger.info(
                f"SNR file exists at {snr_file_path} or {snr_file_path_gz} with sufficient size"
            )
            ...
            try:
                ...
                if not rinex2snr_success:
                    day_logger.error(
                        f"Failed to run rinex2snr for {station_id.upper()} {year} {doy_padded}"
                    )
    ...
    if skip_options.get("skip_rh", False):
        ...
        if check_file_exists(rh_file_path, min_size_bytes=MIN_RH_RESULT_SIZE_BYTES):
            ...
            day_logger.error(
                f"gnssir skipped, but expected result file {rh_file_path} is missing or too small."
            )
            ...
            try:
                ...
                if not gnssir_success:
                    day_logger.error(
                        f"Failed to run gnssir for {station_id.upper()} {year} {doy_padded}"
                    )
            ...
            if plots_in_workspace:
                ...
                for plot_file in plots_in_workspace:
                    ...
                    try:
                        ...
                        day_logger.error(
                            f"Error copying {plot_file} to {target_path}: {copy_error}"
                        )

                result["skipped_steps"].append("quicklook")
            else:
                day_logger.warning(
                    "quickLook skipped, but no plot files found in workspace. Running quickLook..."
                )
                ...
                try:
                    ...
                    result["errors"].append(
                        f"Exception starting quickLook in background thread: {e}"
                    )
        ...
        if gnssrefl_logs_dir.exists():
            for log_pattern in [
                ...
                if log_file_src.exists():
                    ...
                    day_logger.info(
                        f"Copied gnssrefl log file from {log_file_src} to {log_file_dst}"
                    )
    except Exception as e:
