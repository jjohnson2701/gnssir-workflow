...
def plot_resolution_comparison(
    ...
    for res_name, res_code in resolutions:
        ...
        stats.append({"name": res_name, "corr": corr, "rmse": rmse, "n": n_points, "data": data})
    ...
    ax_corr.set_title(
        "Correlation vs Temporal Resolution (Full Year)", fontsize=12, fontweight="bold"
    )
    ...
    bars_rmse = ax_rmse.bar(
        range(len(stats)), rmses, color=colors, edgecolor="black", linewidth=0.5
    )
    ...
    for i, (start_month, end_month, title) in enumerate(quarters):
        ...
        if len(q_data) > 0:
            ...
            q_data["doy_frac"] = (
                q_data["gnss_datetime"].dt.dayofyear
...
def main():
    ...
    station_sources = {"MDAI": "USGS", "FORA": "USGS", "VALR": "CO-OPS", "GLBX": "CO-OPS"}
