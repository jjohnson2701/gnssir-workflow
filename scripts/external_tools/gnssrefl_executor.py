...
def execute_rinex2snr(
    ...
    try:
        ...
        if process_rinex2snr.stderr:
            day_logger.warning(
                f"rinex2snr stderr for DOY {doy_padded}:\n{process_rinex2snr.stderr}"
            )
        ...
        expected_snr_file = (
            ...
            / "snr"
            / station_4char_lower
            / f"{station_4char_lower}{doy_padded}0.{str(year)[-2:]}.snr{snr_code}"
        )
...
def execute_gnssir(
    gnssir_exe_path, station_4char_lower, year, doy_padded, refl_code_base, orbits_base, logs_dir
):
    """
    ...
    try:
        ...
        cmd = [gnssir_exe_path, station_4char_lower, str(year), doy_padded]
        ...
        process_gnssir = subprocess.run(
            cmd, capture_output=True, text=True, shell=False, cwd=cwd_for_gnssir, env=my_env
        )
        ...
        result_file_patterns = [
            ...
            / "results"
            / station_4char_lower
            / f"{station_4char_lower}_{doy_padded}.txt",
        ...
        if not result_file_found:
            ...
            for results_dir in possible_results_dirs:
                if results_dir.exists():
                    ...
                    if txt_files:
                        day_logger.info(
                            f"Found {len(txt_files)} text files in {results_dir}, assuming success"
                        )
        ...
        day_logger.info(
            f"Successfully executed gnssir for {station_4char_lower} {year} {doy_padded}"
        )
        return True
...
def execute_quicklook_threaded(
    quicklook_exe_path,
    station_4char_lower,
    year,
    doy_padded,
    ...
    day_logger = logging.getLogger(str(log_file))
    day_logger.info(
        f"Starting quickLook in background thread for {station_4char_lower} {year} {doy_padded}"
    )
...
def execute_quicklook(
    ...
    try:
        ...
        cmd = [quicklook_exe_path, station_4char_lower, str(year), doy_padded]
        ...
        process_quicklook = subprocess.run(
            cmd, capture_output=True, text=True, shell=False, cwd=cwd_for_quicklook, env=my_env
        )
        ...
        if process_quicklook.stderr:
            day_logger.warning(
                f"quickLook stderr for DOY {doy_padded}:\n{process_quicklook.stderr}"
            )
        ...
        for plot_dir in plot_locations:
            ...
            for pattern in search_patterns:
                ...
                if matching_files:
                    ...
                    day_logger.info(
                        f"Found {len(matching_files)} files matching '{pattern}' in {plot_dir}"
                    )
        ...
        if copied_files:
            day_logger.info(
                f"Successfully copied {len(copied_files)} plot files to {quicklook_plots_daily_dir}"
            )
        else:
            day_logger.warning(
                f"No plot files were copied for {station_4char_lower} {year} {doy_padded}"
            )
            # Not treating this as a failure - the process can continue even without plots

        day_logger.info(
            f"Successfully executed quickLook for {station_4char_lower} {year} {doy_padded}"
        )
        return True
