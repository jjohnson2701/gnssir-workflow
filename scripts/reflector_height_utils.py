...
def calculate_wse_from_rh(gnssir_daily_df, antenna_ellipsoidal_height):
    ...
    logging.info(f"Checking for RH median column in dataset with columns: {gnssir_daily_df.columns.tolist()}")
    ...
    rh_col = None
    for possible_col in ['rh_median_m', 'RH_median_m', 'RH_median', 'rh_median', 'reflector_height_median', 'median']:
    ...
    if rh_col is None:
        ...
        rh_related_cols = [col for col in gnssir_daily_df.columns if 'rh' in col.lower() or 'median' in col.lower()]
    ...
    result_df['wse_ellips_m_demeaned'] = result_df['wse_ellips_m'] - result_df['wse_ellips_m'].mean()
