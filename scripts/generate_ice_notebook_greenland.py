#!/usr/bin/env python3
# ABOUTME: Generates an ice classification Jupyter notebook for Greenland/Arctic stations
# ABOUTME: Adapted from generate_ice_notebook.py (Great Lakes) for multi-GNSS Arctic fjords

"""
Generate an ice classification Jupyter notebook for a Greenland station-year.

Adapted from the Great Lakes generator for Arctic fjord stations that have:
  - Multi-constellation GNSS (GPS + Galileo + GLONASS + BeiDou)
  - No GLERL ground truth — uses Sentinel-1 SAR and Open-Meteo temperature
  - Config-based ice/water season classification (ice_free_months)
  - Known land-contamination sectors within the main azimuth window

Walks through:
  1. Data overview and station context
  2. Sentinel-1 SAR validation
  3. Raw amplitude seasonal signal
  4. SNR feature deep dive with per-season comparison
  5. Feature polarity assessment (vs ROSS/literature)
  6. Per-sector classification behaviour
  7. Phase observable with per-constellation breakdown
  8. Multi-frequency ΔRH analysis (headline: 8+ bands)
  9. ΔRH vs frequency separation (penetration hypothesis test)
  10. MTT freeze/thaw transition detection
  11. Land-water atmospheric comparison
  12. Multi-year overview

Usage:
    python scripts/generate_ice_notebook_greenland.py --station UMNQ --year 2025
    python scripts/generate_ice_notebook_greenland.py --station UMNQ --year 2025 --open
"""
...
def generate_notebook(station, year):
    ...
    # =====================================================================
    cells.append(_md(f"""
# Ice Detection Analysis: {station} {year} — Arctic Fjord

This notebook walks through GNSS-IR ice detection at **{station}** for **{year}**,
an Arctic fjord station with multi-constellation, multi-frequency GNSS data.

Validation uses **Sentinel-1 SAR backscatter** and **Open-Meteo temperature**
in place of GLERL (Great Lakes only). Ice/water seasons are defined from the
station's `ice_free_months` configuration.

The analysis investigates:
- Which GNSS-IR features respond to Arctic fjord ice
- Whether multi-frequency ΔRH reveals frequency-dependent ice penetration (8+ bands)
- Feature polarity: do default settings work, or are ROSS-style inversions needed?
- Per-constellation phase behaviour across GPS, Galileo, and GLONASS
- Land-sector atmospheric comparison using known land-contamination azimuths
"""))
...
Station characteristics are computed dynamically from configuration and data below.
This is a **Greenland/Arctic** station with multi-constellation GNSS and no GLERL ground truth.
Validation relies on Sentinel-1 SAR backscatter and Open-Meteo temperature data.
"""))
...
Load the per-arc retrievals, SNR features, classification results, Sentinel-1 matched data,
...
# Sentinel-1 SAR matched data
s1_path = results_dir / f"{{STATION}}_{{YEAR}}_s1_gnssir_matched.parquet"
s1 = None
if s1_path.exists():
    s1 = pd.read_parquet(s1_path)
    s1["date_dt"] = pd.to_datetime(s1["acquisition_date"])
    s1["month"] = s1["date_dt"].dt.month
    print(f"\\nSentinel-1: {{len(s1)}} matched scenes")
    for fd in s1["flight_direction"].unique():
        n = (s1["flight_direction"] == fd).sum()
        print(f"  {{fd}}: {{n}} scenes")
else:
    print("\\nNo Sentinel-1 matched data.")

# Open-Meteo daily temperature
met_path = results_dir / f"{{STATION}}_{{YEAR}}_met_daily.csv"
met = None
if met_path.exists():
    met = pd.read_csv(met_path, parse_dates=["date"])
    met["date_dt"] = pd.to_datetime(met["date"])
    met["month"] = met["date_dt"].dt.month
    print(f"\\nMet data: {{len(met)}} days, temp range {{met['temp_min_c'].min():.1f}}°C to {{met['temp_max_c'].max():.1f}}°C")
    if "sst_max_c" in met.columns:
        sst_valid = met["sst_max_c"].dropna()
        if len(sst_valid) > 0:
            print(f"  SST range: {{sst_valid.min():.1f}}°C to {{sst_valid.max():.1f}}°C")
else:
    print("\\nNo met data.")

# --- Season classification from config (no GLERL) ---
WATER_MONTHS = sorted(ice_free_months)
_buffer = set()
for m in WATER_MONTHS:
    _buffer.add((m - 2) % 12 + 1)
    _buffer.add(m % 12 + 1)
TRANSITION_MONTHS = sorted(_buffer - set(WATER_MONTHS))
ICE_MONTHS = sorted(set(range(1, 13)) - set(WATER_MONTHS) - set(TRANSITION_MONTHS))

MONTH_TO_SEASON = {{}}
for m in ICE_MONTHS: MONTH_TO_SEASON[m] = "ice"
for m in WATER_MONTHS: MONTH_TO_SEASON[m] = "water"
for m in TRANSITION_MONTHS: MONTH_TO_SEASON[m] = "transition"

print(f"\\nSeason classification (from config ice_free_months):")
print(f"  Ice months:        {{[MONTH_NAMES[m] for m in ICE_MONTHS]}}")
print(f"  Water months:      {{[MONTH_NAMES[m] for m in WATER_MONTHS]}}")
print(f"  Transition months: {{[MONTH_NAMES[m] for m in TRANSITION_MONTHS]}}")

# GLERL not available for Arctic stations
glerl = None

# --- Station metadata (computed from config + data) ---
gnssir_cfg_path = PROJECT_ROOT / "config" / f"{{STATION.lower()}}.json"
with open(gnssir_cfg_path) as _f:
    gnssir_cfg = json.load(_f)
az_lo, az_hi = gnssir_cfg.get("azval2", [0, 360])
configured_freqs = gnssir_cfg.get("freqs", [])
actual_freqs = sorted(per_arc["freq"].unique())
suspect_az = station_cfg.get("suspect_azimuths", {{}})
data_source = station_cfg.get("data_source", "unknown")
print(f"\\nStation metadata:")
print(f"  Azimuth window: {{az_lo:.0f}}°–{{az_hi:.0f}}° ({{az_hi - az_lo:.0f}}° span)")
print(f"  Configured freqs: {{configured_freqs}} ({{len(configured_freqs)}} codes)")
print(f"  Actual freqs in data: {{actual_freqs}} ({{len(actual_freqs)}} codes)")
print(f"  Data source: {{data_source}}")
if suspect_az:
    print(f"  Suspect azimuths: {{suspect_az}}")
"""))
...
Arc density per 10° sector varies because of **multi-GNSS satellite geometry**:

- GPS satellites orbit at ~55° inclination, Galileo at ~56°, GLONASS at ~65°
- Multi-constellation provides denser arc coverage per sector than GPS-only
- Some 10° bins catch 10+ satellites; others fewer
- The classifier requires **≥3 arcs per day** in a sector to classify it
- **Suspect azimuths** (if configured) are flagged but data is preserved for
...
az_bins = np.arange(int(az_lo), int(az_hi) + 1, 1)
...
"""))
    ...
    # =====================================================================
    # Section 2: Sentinel-1 SAR Validation
    # =====================================================================
    cells.append(_md("""
## 2. Sentinel-1 SAR Validation — What does the radar see?

Since GLERL ice concentration is only available for the Great Lakes, we use
**Sentinel-1 SAR backscatter** as external validation for this Arctic station.
S1 scenes are matched to GNSS-IR arcs within a ±2 hour window.

Prior analysis shows Fresnel-zone amp_cv correlates with S1 HH backscatter
at r ≈ +0.83 for descending passes.
"""))

    cells.append(_code(f"""
if s1 is not None and len(s1) > 5:
    fig, axes = plt.subplots(1, 3, figsize=(16, 4.5))

    # Left: S1 backscatter time series
    ax = axes[0]
    for fd, marker, color in [("DESCENDING", "v", "#1565c0"), ("ASCENDING", "^", "#c62828")]:
        mask = s1["flight_direction"] == fd
        if mask.sum() > 0:
            ax.scatter(s1.loc[mask, "date_dt"], s1.loc[mask, "s1_mean_hh_db"],
                       marker=marker, c=color, s=30, alpha=0.7, label=f"{{fd}} (n={{mask.sum()}})")
    # Shade ice/water months
    for m in ICE_MONTHS:
        start = pd.Timestamp(f"{{YEAR}}-{{m:02d}}-01")
        end = start + pd.offsets.MonthEnd(1)
        ax.axvspan(start, end, color="#90caf9", alpha=0.08)
    ax.set_ylabel("S1 HH (dB)")
    ax.set_title("S1 HH Backscatter")
    ax.legend(fontsize=7)

    # Middle: amp_cv vs S1 HH
    ax = axes[1]
    desc = s1[s1["flight_direction"] == "DESCENDING"].copy() if "flight_direction" in s1.columns else s1.copy()
    if "gnssir_amp_cv" in desc.columns and len(desc) > 5:
        season_colors = [MONTH_TO_SEASON.get(m, "transition") for m in desc["month"]]
        c_map = {{"ice": CLASS_COLOR["ice"], "water": CLASS_COLOR["water"], "transition": CLASS_COLOR["transition"]}}
        colors = [c_map.get(s, "#888") for s in season_colors]
        ax.scatter(desc["s1_mean_hh_db"], desc["gnssir_amp_cv"], c=colors, s=40, alpha=0.7)
        r = desc["s1_mean_hh_db"].corr(desc["gnssir_amp_cv"])
        ax.set_xlabel("S1 HH (dB)")
        ax.set_ylabel("GNSS-IR amp_cv")
        ax.set_title(f"Descending: amp_cv vs S1 HH (r={{r:+.3f}})")

    # Right: temperature context
    ax = axes[2]
    if met is not None:
        ax.plot(met["date_dt"], met["temp_mean_c"], color="#333", linewidth=0.8, alpha=0.7, label="Air temp")
        if "sst_max_c" in met.columns:
            ax.plot(met["date_dt"], met["sst_max_c"], color="#1565c0", linewidth=0.8, alpha=0.7, label="SST")
        ax.axhline(0, color="red", ls="--", alpha=0.4, label="0°C freeze proxy")
        ax.set_ylabel("Temperature (°C)")
        ax.set_title("Open-Meteo Temperature")
        ax.legend(fontsize=7)

    fig.suptitle(f"{{STATION}} {{YEAR}} — Validation Context", fontsize=12)
    fig.tight_layout()
    plt.show()
else:
    print("No Sentinel-1 data available for validation.")
    if met is not None:
        fig, ax = plt.subplots(figsize=(14, 3.5))
        ax.plot(met["date_dt"], met["temp_mean_c"], color="#333", linewidth=1, label="Air temp")
        if "sst_max_c" in met.columns:
            ax.plot(met["date_dt"], met["sst_max_c"], color="#1565c0", linewidth=1, label="SST")
        ax.axhline(0, color="red", ls="--", alpha=0.4, label="0°C")
        ax.set_ylabel("Temperature (°C)")
        ax.set_title(f"{{STATION}} {{YEAR}} — Open-Meteo Temperature Context")
        ax.legend()
        fig.tight_layout()
        plt.show()
"""))
...
that matches the expected ice season?
"""))
...
ax1.set_title(f"{STATION} {YEAR} — Daily Mean Amplitude")

# Shade ice/water months
for m in ICE_MONTHS:
    start = pd.Timestamp(f"{YEAR}-{m:02d}-01")
    end = start + pd.offsets.MonthEnd(1)
    if start >= daily_amp["date_dt"].min() and start <= daily_amp["date_dt"].max():
        ax1.axvspan(start, end, color="#90caf9", alpha=0.08)
for m in WATER_MONTHS:
    start = pd.Timestamp(f"{YEAR}-{m:02d}-01")
    end = start + pd.offsets.MonthEnd(1)
    if start >= daily_amp["date_dt"].min() and start <= daily_amp["date_dt"].max():
        ax1.axvspan(start, end, color="#c8e6c9", alpha=0.12)

# Overlay temperature if available
if met is not None:
    ax2 = ax1.twinx()
    ax2.plot(met["date_dt"], met["temp_mean_c"], color="#e53935", linewidth=0.8, alpha=0.4)
    ax2.axhline(0, color="#e53935", ls=":", alpha=0.3)
    ax2.set_ylabel("Air Temp (°C)", color="#e53935", fontsize=8)
    ax2.tick_params(axis="y", colors="#e53935", labelsize=7)
...
The key question is: **do these features actually track ice at this Arctic station?**
These indicators were developed and validated on Greenland fjord ice (Strandberg,
Purnell, Song). This station may match those physics more closely than Great Lakes,
or Arctic fjord ice may differ.
"""))
...
if has_snr:
    ...
    for ax, (feat, (label, high_ice, note)) in zip(axes.flat, features.items()):
        ...
        # Color bars by config-defined season
        bar_colors = []
        for m in monthly.index:
            season = MONTH_TO_SEASON.get(m, "transition")
            bar_colors.append(CLASS_COLOR.get(season, "#f9a825"))
    ...
    fig.suptitle(f"{STATION} {YEAR} — Monthly SNR Feature Medians\\n"
                 f"(Blue = ice months, Green = water months, Yellow = transition)",
                 fontsize=11)
    fig.tight_layout()
    plt.show()
else:
...
"""))
    ...
    # =====================================================================
    cells.append(_md("""
## 5. Feature–Ice Contrast

For each indicator, we compare distributions in config-defined **ice months**
vs **water months** to determine whether it tracks ice presence and what
polarity it operates at.

Positive contrast = feature is HIGHER during ice.
Negative contrast = feature is LOWER during ice.
"""))

    cells.append(_code("""
# Compare features between ice and water months
daily_features = per_arc.groupby("date_dt").agg({
    "Amp": "mean",
    **({feat: "median" for feat in ["CLR", "AF", "PR", "gamma"] if feat in per_arc.columns})
}).reset_index()
daily_features["month"] = daily_features["date_dt"].dt.month

expected_polarity = {
    "Amp": ("positive", True, "High = ice (coherent reflection)"),
    "CLR": ("positive", True, "High = ice (dominant peak)"),
    "AF": ("positive", True, "High = ice (integrated power)"),
    "PR": ("positive", True, "High = ice (single surface)"),
    "gamma": ("negative", False, "Low = ice (smooth surface)"),
}

feature_contrasts = {}
print(f"{'Feature':>10} | {'Ice med':>8} | {'Water med':>9} | {'Direction':>10} | {'Expected':>10} | {'Match?':>6}")
print("-" * 70)

for feat in ["Amp", "CLR", "AF", "PR", "gamma"]:
    if feat not in daily_features.columns:
        continue
    f_ice = daily_features[daily_features["month"].isin(ICE_MONTHS)][feat].dropna()
    f_water = daily_features[daily_features["month"].isin(WATER_MONTHS)][feat].dropna()
    if len(f_ice) < 5 or len(f_water) < 5:
        continue
    ice_med = f_ice.median()
    water_med = f_water.median()
    actual_sign = "positive" if ice_med > water_med else "negative"
    exp_sign = expected_polarity[feat][0]
    match = "YES" if actual_sign == exp_sign else "INVERTED"
    feature_contrasts[feat] = {"ice": ice_med, "water": water_med, "match": match}
    print(f"{feat:>10} | {ice_med:>8.3f} | {water_med:>9.3f} | {actual_sign:>10} | {exp_sign:>10} | {match}")

# Visualize: violin plots for ice vs water
fig, axes = plt.subplots(1, len(feature_contrasts), figsize=(3.2 * len(feature_contrasts), 4))
if len(feature_contrasts) == 1:
    axes = [axes]

for ax, feat in zip(axes, feature_contrasts.keys()):
    f_ice = per_arc[per_arc["month"].isin(ICE_MONTHS)][feat].dropna()
    f_water = per_arc[per_arc["month"].isin(WATER_MONTHS)][feat].dropna()
    parts = ax.violinplot([f_water.values, f_ice.values], positions=[1, 2], showmedians=True)
    for pc, color in zip(parts["bodies"], [CLASS_COLOR["water"], CLASS_COLOR["ice"]]):
        pc.set_facecolor(color)
        pc.set_alpha(0.5)
    ax.set_xticks([1, 2])
    ax.set_xticklabels(["Water", "Ice"])
    match = feature_contrasts[feat]["match"]
    ax.set_title(f"{feat} ({match})", fontsize=10,
                 color="#2e7d32" if match == "YES" else "#c62828")

fig.suptitle(f"{STATION} {YEAR} — Feature Distributions: Ice vs Water Months", fontsize=11)
fig.tight_layout()
plt.show()
"""))
    ...
    # =====================================================================
    cells.append(_md("""
### 5a. Damping parameter (γ) — Smooth or rough ice?
...
At **Arctic fjord stations**, the default polarity (low γ = ice) may hold because fjord
ice is tidal and relatively smooth compared to Great Lakes ice. But this needs empirical
verification — the ice type at this station determines the polarity.
"""))

    cells.append(_code("""
if has_snr and "gamma" in per_arc.columns and ICE_MONTHS and WATER_MONTHS:
    ...
    if len(gamma_ice) > 0 and len(gamma_water) > 0:
        ...
        if ratio > 1.2:
            print(f"γ is {ratio:.1f}x HIGHER during ice months — rough ice (like Great Lakes).")
            print("Feature_overrides may need gamma polarity inverted (high_is_ice=True).")
        elif ratio < 0.8:
            print(f"γ is {ratio:.1f}x LOWER during ice months — smooth ice (standard Greenland expectation).")
            print("Default polarity correct (low γ = ice).")
        else:
            print(f"γ ratio: {ratio:.2f}x — minimal seasonal contrast, feature is not discriminating.")
"""))

    # =====================================================================
    # Section 5c: Temperature context
    # =====================================================================
    cells.append(_md(f"""
### 5b. Temperature Context — Freeze/Thaw Proxy

No NDBC wind buoy is available near this Arctic station. Instead we use
**Open-Meteo** daily temperature data as environmental context. Air temperature
crossing 0°C is a rough freeze-up/break-up proxy for coastal ice.
"""))

    cells.append(_code(f"""
if met is not None and len(met) > 30:
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 7), sharex=True)

    # Top: temperature time series with season shading
    ax1.plot(met["date_dt"], met["temp_mean_c"], color="#333", linewidth=1, label="Air Temp (mean)")
    ax1.fill_between(met["date_dt"], met["temp_min_c"], met["temp_max_c"],
                     color="#ffab91", alpha=0.2, label="Min–Max range")
    if "sst_max_c" in met.columns:
        ax1.plot(met["date_dt"], met["sst_max_c"], color="#1565c0", linewidth=1,
                 alpha=0.7, label="SST (max)")
    ax1.axhline(0, color="red", ls="--", linewidth=1.5, alpha=0.5, label="0°C freeze proxy")
    # Shade ice months
    for m in ICE_MONTHS:
        start = pd.Timestamp(f"{{YEAR}}-{{m:02d}}-01")
        end = start + pd.offsets.MonthEnd(1)
        if start >= met["date_dt"].min() and start <= met["date_dt"].max():
            ax1.axvspan(start, end, color="#90caf9", alpha=0.06)
    ax1.set_ylabel("Temperature (°C)")
    ax1.set_title(f"{{STATION}} {{YEAR}} — Daily Temperature (Open-Meteo)")
    ax1.legend(fontsize=8, loc="upper right")

    # Bottom: classification score with temperature overlay
    ax2.scatter(clf["date_dt"], clf["ice_score"],
                c=[CLASS_COLOR.get(c, "#888") for c in clf["classification"]],
                s=10, alpha=0.6)
    ax2.axhline(-0.33, color="gray", ls=":", alpha=0.5)
    ax2.axhline(0.33, color="gray", ls=":", alpha=0.5)
    ax2.set_ylim(-1.1, 1.1)
    ax2.set_ylabel("Ice Score")
    ax_t = ax2.twinx()
    ax_t.plot(met["date_dt"], met["temp_mean_c"], color="#e53935", linewidth=0.8, alpha=0.4)
    ax_t.axhline(0, color="#e53935", ls=":", alpha=0.3)
    ax_t.set_ylabel("Air Temp (°C)", color="#e53935", fontsize=8)
    ax_t.tick_params(axis="y", colors="#e53935", labelsize=7)
    ax2.set_title("Ice Classification vs Temperature")
    ...
    met["_m"] = met["date_dt"].dt.month
    print("\\nMonthly temperature summary:")
    for m in sorted(met["_m"].unique()):
        md = met[met["_m"] == m]
        season = MONTH_TO_SEASON.get(m, "?")
        print(f"  {{MONTH_NAMES.get(m, str(m)):>3}} ({{season:>5}}): "
              f"mean={{md['temp_mean_c'].mean():>+5.1f}}°C, "
              f"min={{md['temp_min_c'].min():>+5.1f}}°C")
else:
...
for ax, (label, month_filter, color) in zip(axes, [
    ...
    # Azimuth wedge lines
    for az_deg in [int(az_lo), int((az_lo + az_hi) / 2), int(az_hi)]:
...
print(f"Reflection distances over fjord/coastal surface.")
"""))
...
Blue shading indicates config-defined ice months. Temperature overlay
(when available) provides a physical freeze/thaw context.
"""))
...
for m in ICE_MONTHS:
    ...
    if start >= clf["date_dt"].min() and start <= clf["date_dt"].max():
        ax1.axvspan(start, end, color="#90caf9", alpha=0.08)

# Temperature overlay if available
if met is not None:
    ax2 = ax1.twinx()
    ax2.plot(met["date_dt"], met["temp_mean_c"], color="#e53935", linewidth=0.8, alpha=0.35)
    ax2.axhline(0, color="#e53935", ls=":", alpha=0.3)
    ax2.set_ylabel("Air Temp (°C)", color="#e53935", fontsize=8)
    ax2.tick_params(axis="y", colors="#e53935", labelsize=7)

ax1.legend(loc="upper left")
ax1.set_title(f"{STATION} {YEAR} — Ice Classification")
fig.autofmt_xdate()
fig.tight_layout()
plt.show()
"""))
...
Monthly breakdown of classifier output with expected season from config.
"""))
...
for m in sorted(clf["month"].unique()):
    ...
    expected = MONTH_TO_SEASON.get(m, "?")
    majority_class = mc.index[0] if len(mc) > 0 else "?"
    match = "YES" if majority_class == expected else ("~" if expected == "transition" else "MISMATCH")
    row = {
...
"""))
    ...
    # =====================================================================
    # Section 10: S1 SAR Validation
    # =====================================================================
    cells.append(_md("""
## 10. Sentinel-1 SAR Validation

Compare GNSS-IR classification against Sentinel-1 SAR backscatter.
Descending passes show stronger correlation with GNSS-IR observables.
"""))

    cells.append(_code(f"""
if s1 is not None and len(s1) > 10:
    # Merge S1 with classification
    s1_day = s1.copy()
    s1_day["date_dt"] = s1_day["date_dt"].dt.normalize()
    merged = clf[["date_dt", "ice_score", "classification"]].merge(
        s1_day[["date_dt", "s1_mean_hh_db", "flight_direction", "gnssir_amp_cv"]],
        on="date_dt", how="inner")
    print(f"Matched {{len(merged)}} S1-GNSS-IR days")

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 5))

    # Scatter: ice_score vs S1 HH
    colors = [CLASS_COLOR.get(c, "#888") for c in merged["classification"]]
    ax1.scatter(merged["s1_mean_hh_db"], merged["ice_score"], c=colors, s=20, alpha=0.7)
    ax1.axhline(-0.33, color="gray", ls=":", alpha=0.5)
    ax1.axhline(0.33, color="gray", ls=":", alpha=0.5)
    ax1.set_xlabel("S1 HH Backscatter (dB)")
    ax1.set_ylabel("GNSS-IR Ice Score")
    r_all = merged["s1_mean_hh_db"].corr(merged["ice_score"])
    ax1.set_title(f"Ice Score vs S1 HH (r={{r_all:+.3f}})")

    # By flight direction
    for fd in sorted(merged["flight_direction"].unique()):
        sub = merged[merged["flight_direction"] == fd]
        r = sub["s1_mean_hh_db"].corr(sub["ice_score"]) if len(sub) > 5 else float("nan")
        print(f"  {{fd}}: {{len(sub)}} scenes, ice_score vs S1 HH r = {{r:+.3f}}")

    # Confusion: ice vs water season
    bins = {{"Ice months": merged["date_dt"].dt.month.isin(ICE_MONTHS),
            "Transition": merged["date_dt"].dt.month.isin(TRANSITION_MONTHS),
            "Water months": merged["date_dt"].dt.month.isin(WATER_MONTHS)}}
    ...
    print("No S1 data for validation.")
"""))
...
Which indicators are voting which way? For each indicator, we count votes
during **config-defined ice months** vs **water months**.
"""))

    cells.append(_code("""
# Per-sector vote columns
...
print(f"Per-sector indicator types: {sorted(indicator_types)}")

clf_ice = clf[clf["month"].isin(ICE_MONTHS)]
clf_water = clf[clf["month"].isin(WATER_MONTHS)]

print(f"\\n{'Indicator':>10} | {'Ice months':>30} | {'Water months':>30}")
...
for ind in sorted(indicator_types):
    ...
    for _, row in clf_water.iterrows():
        for col in ind_vote_cols:
            v = row.get(col)
...
"""))
    ...
    # =====================================================================
    cells.append(_md("""
## 12. CLR and PR Assessment — Does multi-GNSS help?

**CLR** (Clarity Ratio) and **PR** (Peak Ratio) measure how dominant the primary LSP
peak is. At ROSS (GPS-only, 2 bands), these features were weak because the LSP
structure barely changed between ice and water.

This station has **multi-GNSS** data with up to 10 frequencies, which is exactly what
CLR and PR were designed for. If Arctic fjord ice produces a cleaner single-surface
reflection than open water, CLR and PR should work at their original (positive) polarity.
"""))

    cells.append(_code("""
# CLR/PR seasonal contrast analysis with config-based seasons
if has_snr and ICE_MONTHS and WATER_MONTHS:
    for feat, label in [("CLR", "Clarity Ratio"), ("PR", "Peak Ratio")]:
        ...
        overlap_pct = max(0, overlap_pct) / max(iqr, 1e-6) * 100

        print(f"{label} ({feat}):")
        print(f"  Ice months {ICE_MONTHS}:   median={f_ice.median():.2f}  IQR=[{f_ice.quantile(0.25):.2f}, {f_ice.quantile(0.75):.2f}]")
        print(f"  Water months {WATER_MONTHS}: median={f_water.median():.2f}  IQR=[{f_water.quantile(0.25):.2f}, {f_water.quantile(0.75):.2f}]")
        print(f"  Separation: {sep:.2f} — {'WITHIN' if sep < iqr else 'EXCEEDS'} water IQR of {iqr:.2f}")
        direction = "HIGHER in ice" if f_ice.median() > f_water.median() else "LOWER in ice"
    ...
    if len(freqs) >= 6:
        print(f"  → Rich spectral diversity — CLR/PR should be effective")
    else:
        print(f"  → Limited spectral diversity — CLR/PR may be weak")
"""))
    ...
    # =====================================================================
    cells.append(_md("""
## 13. Feature Overrides Assessment

The classifier supports station-specific **feature overrides** for adjusting polarity
and weights. At ROSS (Great Lakes), γ/CLR/PR needed polarity inversion because rough
ice behaved opposite to the literature.

This Arctic station uses **default polarities** (no overrides). This section shows
the current configuration and compares the default vs any overridden classification.
"""))
...
for label, c in [("default", clf_default), ("current", clf_current)]:
    ...
    c["month"] = c["date_dt"].dt.month

m_def = clf_default
m_cur = clf_current

# Season-based accuracy
print(f"  {{'Metric':<40}} {{'Default':>10}} {{'Current':>10}}")
print(f"  {{'-'*65}}")
for label, months, target_cls in [
    ("Ice months: classified ice", ICE_MONTHS, "ice"),
    ("Water months: classified water", WATER_MONTHS, "water"),
]:
    d_sub = m_def[m_def["month"].isin(months)]
    c_sub = m_cur[m_cur["month"].isin(months)]
    d_n = (d_sub["classification"] == target_cls).sum()
    c_n = (c_sub["classification"] == target_cls).sum()
    d_t = len(d_sub)
    c_t = len(c_sub)
...
for ax, (label, m_data) in zip([ax1, ax2], [("Default (no overrides)", m_def),
    ...
    for m in ICE_MONTHS:
        ...
        if start >= m_data["date_dt"].min() and start <= m_data["date_dt"].max():
            ax.axvspan(start, end, color="#90caf9", alpha=0.06)
...
This is v1 of the Arctic fjord ice analysis. Key questions being addressed:

1. **Do default feature polarities work?** (Section 5 polarity assessment above)
2. **Does multi-frequency ΔRH reveal ice penetration?** (Section 16+ below)
3. **Is phase a stronger discriminator with multi-GNSS?** (Section 15 below)
4. **Does the land-sector atmospheric comparison work?** (Section 18 below)

| Indicator | Literature (smooth ice) | ROSS (rough GL ice) | This station |
|-----------|------------------------|--------------------:|:-------------|
| **γ** (damping) | Low = ice | High = ice (INVERTED) | TBD (see Section 5a) |
| **CLR** (clarity) | High = ice | Low = ice (INVERTED) | TBD |
| **PR** (peak ratio) | High = ice | Low = ice (INVERTED) | TBD |
| **Amp** (amplitude) | High = ice | High = ice (standard) | TBD |
| **AF** (area factor) | High = ice | High = ice (standard) | TBD |

Feature polarity results from Section 5 should be used to populate this table
and determine whether `feature_overrides` are needed for this station.
"""))
...
if has_snr and "phase" in per_arc.columns:
    ...
    ax = axes[1, 0]
    phase_ice = per_arc[per_arc["month"].isin(ICE_MONTHS)]["phase"].dropna()
    phase_water = per_arc[per_arc["month"].isin(WATER_MONTHS)]["phase"].dropna()
    if True:
    ...
    # Bottom-right: phase vs ice_score scatter
    ax = axes[1, 1]
    dp = daily_phase.copy()
    dp = dp.merge(clf[["date_dt", "ice_score"]], on="date_dt", how="inner")
    valid = dp.dropna(subset=["phase_mean", "ice_score"])
    if len(valid) > 20:
        r = valid["phase_mean"].corr(valid["ice_score"])
        ax.scatter(valid["ice_score"], valid["phase_mean"], s=8, alpha=0.4, c="#555")
        ax.set_xlabel("Ice Score")
        ax.set_ylabel("Daily Circular Mean Phase (rad)")
        ax.set_title(f"Phase vs Ice Score: r={r:+.3f}")
    else:
    ...
    if len(phase_ice) > 0 and len(phase_water) > 0:
...
The overall ice-water phase shift is a station-level average.
...
"""))
    ...
    cells.append(_code("""
if has_snr and "phase" in per_arc.columns and ICE_MONTHS and WATER_MONTHS:
    ice_m = ICE_MONTHS
    water_m = WATER_MONTHS
    ...
    if prn_shifts:
        ...
        ax = axes[2]
        _PH_BAND = {
            1: "L1", 2: "L2", 20: "L2C", 5: "L5",
            101: "E1", 102: "E5a", 105: "E5a",
            201: "G1", 205: "G2",
            206: "E5b", 207: "E5", 208: "E6",
            302: "B1", 306: "B3",
        }
        per_arc["_phband"] = per_arc["freq"].map(_PH_BAND)
        _band_colors = ["#e53935", "#1e88e5", "#43a047", "#8e24aa",
                         "#ff6f00", "#00897b", "#5c6bc0", "#c62828",
                         "#2e7d32", "#f57f17", "#4527a0", "#00695c"]
        detected_bands = sorted(per_arc["_phband"].dropna().unique())
        band_shifts = {}
        for band, color in zip(detected_bands, _band_colors[:len(detected_bands)]):
        ...
        daily_circ_std["month"] = daily_circ_std["date_dt"].dt.month

        cs_ice = daily_circ_std[daily_circ_std["month"].isin(ice_m)]["phase_cstd"].dropna()
        cs_water = daily_circ_std[daily_circ_std["month"].isin(water_m)]["phase_cstd"].dropna()
    ...
    print("Per-satellite phase analysis requires SNR features with phase data.")
"""))
...
This station has **multi-GNSS** data with 8+ distinct frequency bands spanning
1176–1602 MHz. If the ice penetration hypothesis is correct, the ΔRH signal should
be dramatically clearer here than at GPS-only stations like ROSS.

ΔRH is computed per (date, azimuth sector) as the standard deviation of per-band RH medians.
"""))

    cells.append(_code("""
# Compute per-sector daily ΔRH
FREQ_TO_BAND = {
    1: "L1", 2: "L2", 20: "L2C", 5: "L5",
    101: "E1", 102: "E5a", 105: "E5a",
    201: "G1", 205: "G2",
    206: "E5b", 207: "E5", 208: "E6",
    301: "B1", 302: "B1", 306: "B3", 307: "B2a",
}
BAND_FREQ_MHZ = {
    "L1": 1575.42, "L2": 1227.60, "L2C": 1227.60, "L5": 1176.45,
    "E1": 1575.42, "E5a": 1176.45, "E5b": 1207.14, "E5": 1191.795, "E6": 1278.75,
    "G1": 1602.0, "G2": 1246.0,
    "B1": 1561.098, "B2a": 1176.45, "B3": 1268.52,
}

per_arc["_band"] = per_arc["freq"].map(FREQ_TO_BAND)
n_bands = per_arc["_band"].nunique()
print(f"Frequency bands present: {sorted(per_arc['_band'].dropna().unique())} ({n_bands} bands)")
...
if n_bands >= 2:
    ...
    for (date, az_bin), grp in per_arc.groupby(["date_dt", "azimuth_bin"]):
        ...
        if len(band_rh) >= 2:
            drh_rows.append({
                ...
                "n_bands": len(band_rh),
                "band_range": float(band_rh.max() - band_rh.min()),
            })
    drh_df = pd.DataFrame(drh_rows)
    print(f"ΔRH computed for {len(drh_df)} (date, sector) pairs")

    # Station-level daily ΔRH
    daily_drh = drh_df.groupby("date_dt")["delta_rh"].median().reset_index()
    ...
    ax = axes[1, 0]
    drh_ice = daily_drh[daily_drh["month"].isin(ICE_MONTHS)]["delta_rh"]
    drh_water = daily_drh[daily_drh["month"].isin(WATER_MONTHS)]["delta_rh"]
    if True:
    ...
    # Bottom-right: ΔRH vs ice score scatter
    ax = axes[1, 1]
    m_drh_clf = daily_drh.merge(clf[["date_dt", "ice_score"]], on="date_dt", how="inner")
    valid = m_drh_clf.dropna(subset=["delta_rh", "ice_score"])
    if len(valid) > 20:
        r = valid["delta_rh"].corr(valid["ice_score"])
        ax.scatter(valid["ice_score"], valid["delta_rh"], s=8, alpha=0.4, c="#555")
        ax.set_xlabel("Ice Score")
        ax.set_ylabel("ΔRH (m)")
        ax.set_title(f"ΔRH vs Ice Score: r={r:+.3f}")

    fig.suptitle(f"{STATION} {YEAR} — Interfrequency ΔRH Analysis\\n"
                 f"({n_bands} bands: {sorted(per_arc['_band'].dropna().unique())})", fontsize=12)
    fig.tight_layout()
    plt.show()

    # Quantitative summary
    if len(drh_ice) > 0 and len(drh_water) > 0:
        ...
        if ratio > 1.2:
            ...
            print(f"  → Minimal contrast with {n_bands} bands — ice penetration hypothesis not supported at this station.")
            print("  → ΔRH does not discriminate ice from water even with multi-frequency data.")
else:
    print(f"Only {n_bands} frequency band(s) available — ΔRH requires ≥2. Skipping.")
"""))
...
"""))
    ...
    cells.append(_code("""
# --- L1 vs L2C RH scatter plot ---
# Use the corrected FREQ_TO_BAND from Section 16
per_arc["_band_v"] = per_arc["freq"].map(FREQ_TO_BAND)

# Pick the two most common bands for the scatter
band_counts = per_arc["_band_v"].value_counts()
top_bands = band_counts.index[:2].tolist()
b1_name, b2_name = top_bands[0], top_bands[1]

b1_arcs = per_arc[per_arc["_band_v"] == b1_name][["date", "date_dt", "sat", "RH", "month"]].copy()
b1_arcs = b1_arcs.rename(columns={"RH": "RH_B1"})
b2_arcs = per_arc[per_arc["_band_v"] == b2_name][["date", "sat", "RH"]].copy()
b2_arcs = b2_arcs.rename(columns={"RH": "RH_B2"})

matched = b1_arcs.merge(b2_arcs, on=["date", "sat"])
print(f"{b1_name}-{b2_name} matched pairs: {len(matched)} (by date + satellite)")

ice_m_v = ICE_MONTHS
water_m_v = WATER_MONTHS
...
if len(matched) > 50:
    ...
    if other_mask_v.sum() > 0:
        ax.scatter(matched.loc[other_mask_v, "RH_B1"], matched.loc[other_mask_v, "RH_B2"],
                   s=5, alpha=0.15, c="#888", label=f"Other ({other_mask_v.sum()})")
    if water_mask_v.sum() > 0:
        ax.scatter(matched.loc[water_mask_v, "RH_B1"], matched.loc[water_mask_v, "RH_B2"],
                   s=8, alpha=0.3, c=CLASS_COLOR["water"], label=f"Water ({water_mask_v.sum()})")
    if ice_mask_v.sum() > 0:
        ax.scatter(matched.loc[ice_mask_v, "RH_B1"], matched.loc[ice_mask_v, "RH_B2"],
                   s=8, alpha=0.3, c=CLASS_COLOR["ice"], label=f"Ice ({ice_mask_v.sum()})")

    rh_lo = matched[["RH_B1", "RH_B2"]].min().min()
    rh_hi = matched[["RH_B1", "RH_B2"]].max().max()
    ax.plot([rh_lo, rh_hi], [rh_lo, rh_hi], "k--", alpha=0.4, label="1:1 line")

    ice_offset = (matched.loc[ice_mask_v, "RH_B2"].median() -
                  matched.loc[ice_mask_v, "RH_B1"].median()) if ice_mask_v.sum() > 10 else np.nan
    water_offset = (matched.loc[water_mask_v, "RH_B2"].median() -
                    matched.loc[water_mask_v, "RH_B1"].median()) if water_mask_v.sum() > 10 else np.nan
    ...
    print(f"{b1_name}-{b2_name} scatter: insufficient matched pairs or no season data.")
else:
    print("Insufficient band-pair matched pairs for validation.")
"""))

    # =====================================================================
    # Section 16b: Per-Band RH Time Series (multi-frequency headline)
    # =====================================================================
    cells.append(_md("""
### 16b. Per-Band RH Time Series — Do bands diverge during ice?

With 8+ distinct frequency bands, we can test the penetration hypothesis directly:
if ice causes frequency-dependent penetration, each band should measure a slightly
different RH during ice months, but converge during open water.
"""))

    cells.append(_code("""
if n_bands >= 4:
    # Daily median RH per band
    band_daily_rh = per_arc.groupby(["date_dt", "_band"])["RH"].median().unstack("_band")
    detected_bands_rh = sorted(band_daily_rh.columns)

    fig, ax = plt.subplots(figsize=(14, 5))
    _colors = plt.cm.tab10(np.linspace(0, 1, len(detected_bands_rh)))
    for i, band in enumerate(detected_bands_rh):
        series = band_daily_rh[band].dropna()
        rolling = series.rolling(7, center=True, min_periods=3).median()
        ax.plot(rolling.index, rolling.values, linewidth=1, alpha=0.7,
                color=_colors[i], label=f"{band} ({BAND_FREQ_MHZ.get(band, '?'):.0f} MHz)")

    # Shade ice/water months
    for m in ICE_MONTHS:
        start = pd.Timestamp(f"{YEAR}-{m:02d}-01")
        end = start + pd.offsets.MonthEnd(1)
        if start >= band_daily_rh.index.min() and start <= band_daily_rh.index.max():
            ax.axvspan(start, end, color="#90caf9", alpha=0.06)
    for m in WATER_MONTHS:
        start = pd.Timestamp(f"{YEAR}-{m:02d}-01")
        end = start + pd.offsets.MonthEnd(1)
        if start >= band_daily_rh.index.min() and start <= band_daily_rh.index.max():
            ax.axvspan(start, end, color="#c8e6c9", alpha=0.08)

    ax.set_ylabel("RH (m)")
    ax.set_title(f"{STATION} {YEAR} — Per-Band Daily Median RH (7-day rolling)")
    ax.legend(fontsize=7, ncol=3, loc="upper right")
    fig.autofmt_xdate()
    fig.tight_layout()
    plt.show()

    # Per-band ice vs water RH
    print(f"\\n{'Band':>6} | {'Freq MHz':>8} | {'Ice RH':>8} | {'Water RH':>9} | {'Offset':>8}")
    print("-" * 55)
    for band in detected_bands_rh:
        freq = BAND_FREQ_MHZ.get(band, 0)
        rh_ice = band_daily_rh.loc[band_daily_rh.index.month.isin(ICE_MONTHS), band].dropna()
        rh_water = band_daily_rh.loc[band_daily_rh.index.month.isin(WATER_MONTHS), band].dropna()
        if len(rh_ice) > 5 and len(rh_water) > 5:
            offset = rh_ice.median() - rh_water.median()
            print(f"{band:>6} | {freq:>8.1f} | {rh_ice.median():>8.3f} | {rh_water.median():>9.3f} | {offset:>+8.4f}")
else:
    print(f"Only {n_bands} bands — multi-frequency RH analysis requires >=4. Skipping.")
"""))

    # =====================================================================
    # Section 16c: ΔRH vs Frequency Separation (signature plot)
    # =====================================================================
    cells.append(_md("""
### 16c. ΔRH vs Frequency Separation — The Penetration Test

If frequency-dependent ice penetration exists, the RH difference between two bands
should be proportional to their frequency separation during ice, but flat during water.

For each band pair: compute |median_RH_band1 − median_RH_band2| during ice and water
months separately. Plot against |freq1 − freq2| in MHz. A positive trend during ice
with no trend during water = penetration confirmed.
"""))
...
if n_bands >= 4:
    ice_mask_drh = band_daily_rh.index.month.isin(ICE_MONTHS)
    water_mask_drh = band_daily_rh.index.month.isin(WATER_MONTHS)

    pair_results = []
    for b1, b2 in combinations(detected_bands_rh, 2):
        f1, f2 = BAND_FREQ_MHZ.get(b1, 0), BAND_FREQ_MHZ.get(b2, 0)
        freq_sep = abs(f1 - f2)

        # Ice months
        rh1_ice = band_daily_rh.loc[ice_mask_drh, b1].dropna()
        rh2_ice = band_daily_rh.loc[ice_mask_drh, b2].dropna()
        common_ice = rh1_ice.index.intersection(rh2_ice.index)
        ice_rh_diff = abs((rh1_ice.loc[common_ice] - rh2_ice.loc[common_ice]).median()) if len(common_ice) > 10 else np.nan

        # Water months
        rh1_water = band_daily_rh.loc[water_mask_drh, b1].dropna()
        rh2_water = band_daily_rh.loc[water_mask_drh, b2].dropna()
        common_water = rh1_water.index.intersection(rh2_water.index)
        water_rh_diff = abs((rh1_water.loc[common_water] - rh2_water.loc[common_water]).median()) if len(common_water) > 10 else np.nan

        pair_results.append({
            "b1": b1, "b2": b2, "freq_sep_mhz": freq_sep,
            "ice_rh_diff": ice_rh_diff, "water_rh_diff": water_rh_diff,
        })

    pairs_df = pd.DataFrame(pair_results).dropna()

    if len(pairs_df) > 3:
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.scatter(pairs_df["freq_sep_mhz"], pairs_df["ice_rh_diff"],
                   c=CLASS_COLOR["ice"], s=50, alpha=0.7, label="Ice months", zorder=3)
        ax.scatter(pairs_df["freq_sep_mhz"], pairs_df["water_rh_diff"],
                   c=CLASS_COLOR["water"], s=50, alpha=0.7, label="Water months", zorder=3)

        # Trend lines
        for data, color, label in [(pairs_df[["freq_sep_mhz", "ice_rh_diff"]].dropna(), CLASS_COLOR["ice"], "ice"),
                                    (pairs_df[["freq_sep_mhz", "water_rh_diff"]].dropna(), CLASS_COLOR["water"], "water")]:
            if len(data) > 3:
                z = np.polyfit(data.iloc[:, 0], data.iloc[:, 1], 1)
                xl = np.linspace(data.iloc[:, 0].min(), data.iloc[:, 0].max(), 50)
                ax.plot(xl, np.polyval(z, xl), "--", color=color, alpha=0.5,
                        label=f"{label} slope: {z[0]:.1e} m/MHz")

        # Annotate top pairs
        for _, row in pairs_df.nlargest(5, "ice_rh_diff").iterrows():
            ax.annotate(f"{row['b1']}-{row['b2']}", (row["freq_sep_mhz"], row["ice_rh_diff"]),
                        fontsize=7, alpha=0.7)

        ax.set_xlabel("Frequency Separation (MHz)")
        ax.set_ylabel("|Median ΔRH| between bands (m)")
        ax.set_title(f"{STATION} {YEAR} — ΔRH vs Frequency Separation\\n"
                     f"Positive ice slope = frequency-dependent penetration")
        ax.legend(fontsize=9)
        fig.tight_layout()
        plt.show()

        # Summary
        ice_slope = np.polyfit(pairs_df["freq_sep_mhz"], pairs_df["ice_rh_diff"], 1)[0] if len(pairs_df) > 3 else 0
        water_slope = np.polyfit(pairs_df["freq_sep_mhz"], pairs_df["water_rh_diff"], 1)[0] if len(pairs_df) > 3 else 0
        print(f"\\nΔRH vs freq separation slopes:")
        print(f"  Ice months:   {ice_slope:.2e} m/MHz {'(+) supports penetration' if ice_slope > 0 else '(-) no penetration signal'}")
        print(f"  Water months: {water_slope:.2e} m/MHz {'(flat as expected)' if abs(water_slope) < abs(ice_slope) * 0.5 else '(unexpected non-zero)'}")
else:
    print("Insufficient bands for frequency separation analysis.")
"""))
...
# Also run on individual features
feature_t = {}
...
"""))
    ...
    # =====================================================================
    cells.append(_md(f"""
## 18. Land-Water Atmospheric Comparison — Exploratory

**Hypothesis:** During open water season, water-facing arcs traverse atmosphere
with higher moisture content (evaporation). Land-facing arcs traverse drier air.
This differential should be visible in phase and amplitude. During freeze-up,
evaporation stops and the differential collapses toward zero.

**If this convergence precedes ice detection, it's a leading indicator.**

This station has a known **land-contamination sector** (40°–60°) within its main
azimuth window (40°–185°). No reprocessing is needed — the land sector data is
already in the per-arc parquet.

**Sector classification:**
- **Land:** 40°–60° (config suspect_azimuths: land contamination)
- **Coastal buffer:** 60°–70°
- **Water:** 70°–185°

**Caveats:**
- Arctic land surface has its own seasonal cycle (snow, permafrost active layer)
- Fjord geometry may limit the atmospheric path length differential
...
if has_land_data:
    ...
    print("\\nLand sector should be within the main azimuth window for this station.")
    print("Check that the per-arc data includes azimuths in the 40-60 degree range.")
"""))

    cells.append(_code(f"""
if pa_land is not None:
    # Classify arcs by surface type (station-specific)
    # Land: 40-60 (suspect_azimuths), Water: 70-185, Coastal: 60-70 buffer
    ...
    if counts.get("land", 0) < 100:
        ...
        ax = axes[0]
        ax.plot(diff["date_dt"], diff["amp_water"], "b-", linewidth=1, alpha=0.6, label="Water (70°–185°)")
        ax.plot(diff["date_dt"], diff["amp_land"], "r-", linewidth=1, alpha=0.6, label="Land (40°–60°)")
        ...
        # Statistical test: does the differential change between seasons?
        if "delta_amp" in diff.columns:
...
surface. Per-satellite and per-constellation disaggregation in Section 15a.

**Interfrequency ΔRH:** Per-sector computation of frequency-group RH spread.
With 8+ multi-GNSS bands spanning 1176–1602 MHz, this station provides the
frequency diversity needed to test the ice penetration hypothesis. Sections 16b-16c
present the per-band RH analysis and ΔRH vs frequency separation test.

**Freeze/Thaw transition detection (MTT):** The Moving t-Test with constrained
phenology windows (FUS: DOY 270-30, BUS: DOY 120-210 for Arctic stations).
Per-feature detection identifies the earliest transition indicator.

**Land-water atmospheric comparison:** Uses the known land-contamination sector
(40°-60°) vs water sectors (70°-185°) — no expanded-azimuth reprocessing needed.
Tests whether evaporation cessation at freeze-up is detectable.

**Greenland notebook v1:** Adapted from Great Lakes generator. Uses config-based
ice/water months, S1 SAR and Open-Meteo temperature for validation instead of
GLERL. Corrected FREQ_TO_BAND mapping for multi-GNSS (10 distinct bands vs 3
collapsed bands in original mapping).
"""))
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate ice classification Jupyter notebook for Greenland/Arctic stations")
