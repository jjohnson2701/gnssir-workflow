...
class NOAACOOPSClient:
    ...
    def __init__(self, base_url: str = "https://api.tidesandcurrents.noaa.gov/api/prod/datagetter", cache_dir: Optional[Path] = None):
        """
    ...
    def _generate_cache_key(self, product: str, station_id: str, start_date: datetime, 
                           end_date: datetime, **kwargs) -> str:
    ...
    def find_nearby_stations(
        ...
        """
        self.logger.info(f"Searching for CO-OPS stations near ({latitude}, {longitude}) within {radius_km} km")
        
        try:
            # Use metadata API to get stations
            url = f"{self.metadata_url}/stations.json"
            params = {
                'type': 'waterlevels',  # Focus on water level stations
                'expand': 'details'
            }
            ...
            for station in stations_data.get('stations', []):
                try:
                    ...
                    distance_km = self._calculate_distance(latitude, longitude, station_lat, station_lon)
    ...
    def get_tide_predictions(
        ...
        """
        self.logger.info(f"Fetching tide predictions for station {station_id} from {start_date} to {end_date}")
        
        # Check cache first
        cache_key = self._generate_cache_key(
            'predictions', station_id, start_date, end_date,
            datum=datum, interval=interval, time_zone=time_zone, units=units
        )
    ...
    def get_water_level_observations(
        ...
        """
        self.logger.info(f"Fetching water level observations for station {station_id} from {start_date} to {end_date}")
        
        # Check if we can get all data from cache
        full_cache_key = self._generate_cache_key(
            'water_level', station_id, start_date, end_date,
            datum=datum, interval=interval, time_zone=time_zone, units=units
        )
        ...
        if cached_data is not None:
            ...
            if date_diff > 31:
                self.logger.warning(f"Date range ({date_diff} days) exceeds CO-OPS limit of 31 days for water level data. Fetching in chunks...")
            ...
            while current_date < end_date:
                ...
                chunk_cache_key = self._generate_cache_key(
                    'water_level', station_id, current_date, chunk_end,
                    datum=datum, interval=interval, time_zone=time_zone, units=units
                )
                ...
                if chunk_cached_data is not None and 'data' in chunk_cached_data:
                    ...
                    try:
                        ...
                        self.logger.warning(f"Failed to fetch chunk from {current_date} to {chunk_end}: {e}")
    ...
    def get_high_low_tides(
        ...
        """
        self.logger.info(f"Fetching high/low tides for station {station_id} from {start_date} to {end_date}")
        ...
        try:
            ...
            # Clean up columns
            df = df[['datetime', 'height_m', 'tide_type', 'station_id', 'datum', 'units', 'data_type']]
    ...
    def interpolate_to_timestamps(
        ...
        try:
            ...
            continuous_data[value_column] = continuous_data[value_column].interpolate(method='cubic')
