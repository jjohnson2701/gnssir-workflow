...
class NDBCClient:
    ...
    def find_nearby_buoys(
        self, latitude: float, longitude: float, radius_km: float = 200
    ) -> List[Dict]:
        """
        ...
        """
        self.logger.info(
            f"Searching for NDBC buoys near ({latitude}, {longitude}) within {radius_km} km"
        )
        ...
        known_buoys = [
            ...
            {
                "id": "44009",
                "name": "Delaware Bay 26 NM Southeast of Cape Henlopen, DE",
                "lat": 38.461,
                "lon": -74.703,
            },
            {
                "id": "41025",
                "name": "Diamond Shoals 40 NM Southeast of Cape Hatteras, NC",
                "lat": 35.006,
                "lon": -75.402,
            },
            {
                "id": "41013",
                "name": "Frying Pan Shoals 20 NM Southeast of Cape Fear, NC",
                "lat": 33.436,
                "lon": -77.743,
            },
            {
                "id": "44007",
                "name": "Portland 12 NM East of Portsmouth, NH",
                "lat": 43.525,
                "lon": -70.141,
            },
            {
                "id": "44008",
                "name": "Nantucket 54 NM Southeast of Nantucket, MA",
                "lat": 40.504,
                "lon": -69.247,
            },
            {
            ...
            {
                "id": "44017",
                "name": "Montauk Point 19 NM South of Montauk Point, NY",
                "lat": 40.691,
                "lon": -72.048,
            },
            {
                "id": "44020",
                "name": "Nantucket Sound 24 NM Southeast of Nantucket, MA",
                "lat": 41.443,
                "lon": -70.279,
            },
        ]
        ...
        for buoy in known_buoys:
            try:
                distance_km = self._calculate_distance(
                    latitude, longitude, buoy["lat"], buoy["lon"]
                )
    ...
    def get_meteorological_data(self, station_id: str, days_back: int = 45) -> pd.DataFrame:
        """
        ...
        """
        self.logger.info(
            f"Fetching meteorological data for station {station_id} ({days_back} days)"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Retrieved {len(df)} meteorological observations for station {station_id}"
            )
            return df
    ...
    def get_wave_data(self, station_id: str, days_back: int = 45) -> pd.DataFrame:
        """
    ...
    def get_continuous_winds(self, station_id: str, days_back: int = 45) -> pd.DataFrame:
        """
        ...
        """
        self.logger.info(
            f"Fetching continuous wind data for station {station_id} ({days_back} days)"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Retrieved {len(df)} continuous wind observations for station {station_id}"
            )
            return df
    ...
    def interpolate_to_timestamps(
        self, ndbc_data: pd.DataFrame, target_timestamps: pd.Series, value_columns: List[str] = None
    ) -> pd.DataFrame:
        """
            ...
            for col in value_columns:
                if col in continuous_data.columns:
                    ...
                    for timestamp in target_timestamps:
                        closest_idx = continuous_data.index.get_indexer(
