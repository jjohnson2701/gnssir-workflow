...
def validate_configuration(station: str, station_config: dict, project_root: Path) -> dict:
    ...
    refl_code = os.environ.get(
        "REFL_CODE", str(project_root / "gnssrefl_data_workspace" / "refl_code")
    )
...
Examples:
  ...
  python scripts/process_station.py --station GLBX --year 2024 --validate
    ...
    parser.add_argument(
        "--year", type=int, default=None, help="Year to process (default: current year)"
    )
    ...
    parser.add_argument(
        "--days", type=int, default=None, help="Process last N days (handles year boundaries)"
    )
    parser.add_argument(
        "--num_cores", type=int, default=8, help="Number of parallel cores (default: 8)"
    )
    parser.add_argument(
        "--skip_download", action="store_true", help="Skip RINEX download (data already present)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Validate configuration and exit without processing"
    )
    ...
    if not args.skip_gnssir:
        ...
        try:
            ...
            processing_config = load_station_config(
                DEFAULT_STATIONS_CONFIG_PATH, args.station.upper(), project_root
            )
            ...
            logging.info("=" * 80)
            logging.info(
                f"GNSS-IR Processing completed at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
    ...
    if not args.skip_comparison:
        ...
        if ref_source == "erddap":
            cmd = [
                python_exe,
                str(script_dir / "generate_erddap_matched.py"),
                "--station",
                args.station,
                "--year",
                str(args.year),
            ...
            cmd = [
                python_exe,
                str(script_dir / "usgs_comparison.py"),
                "--station",
                args.station,
                "--year",
                str(args.year),
            ...
            cmd = [
                python_exe,
                str(script_dir / "coops_comparison.py"),
                "--station",
                args.station,
                "--year",
                str(args.year),
    ...
    if not args.skip_viz:
        ...
        cmd = [
            python_exe,
            str(script_dir / "plot_resolution_comparison.py"),
            "--station",
            args.station,
            "--year",
            str(args.year),
        ...
        cmd = [
            python_exe,
            str(script_dir / "create_polar_animation.py"),
            "--station",
            args.station,
            "--year",
            str(args.year),
